    ...
    public static void solidify(NativeImage nativeImage) {
        ...
        for (k = 0; k < i; ++k) {
            for (l = 0; l < j; ++l) {
                ...
                if (ARGB.alpha((int)m) == 0) continue;
        ...
        for (k = 0; k < i; ++k) {
            for (l = 0; l < j; ++l) {
                m = nativeImage.getPixel(k, l);
                if (ARGB.alpha((int)m) == 0) {
                    nativeImage.setPixel(k, l, ARGB.color((int)0, (int)is[TextureUtil.pack(k, l, i)]));
                    continue;
                }
    ...
    public static void fillEmptyAreasWithDarkColor(NativeImage nativeImage) {
        ...
        for (m = 0; m < i; ++m) {
            for (n = 0; n < j; ++n) {
                ...
                p = ARGB.alpha((int)o);
                if (p == 0 || (t = (q = ARGB.red((int)o)) + (r = ARGB.green((int)o)) + (s = ARGB.blue((int)o))) >= l) continue;
                l = t;
                k = o;
            }
        }
        m = 3 * ARGB.red((int)k) / 4;
        n = 3 * ARGB.green((int)k) / 4;
        o = 3 * ARGB.blue((int)k) / 4;
        p = ARGB.color((int)0, (int)m, (int)n, (int)o);
        for (q = 0; q < i; ++q) {
            for (r = 0; r < j; ++r) {
                s = nativeImage.getPixel(q, r);
                if (ARGB.alpha((int)s) != 0) continue;
