...
extends RecipeBook {
    ...
    public void rebuildCollections() {
        ...
        map.forEach((recipeBookCategory, list) -> map2.put((SearchRecipeBookCategory)((Object)recipeBookCategory), (List)list.stream().map(RecipeCollection::new).peek(arg_0 -> ((ImmutableList.Builder)builder).add(arg_0)).collect(ImmutableList.toImmutableList())));
    ...
    private static Map<RecipeBookCategory, List<List<RecipeDisplayEntry>>> categorizeAndGroupRecipes(Iterable<RecipeDisplayEntry> iterable) {
        ...
        for (RecipeDisplayEntry recipeDisplayEntry : iterable) {
            ...
            if (optionalInt.isEmpty()) {
                map.computeIfAbsent(recipeBookCategory2, recipeBookCategory -> new ArrayList()).add(List.of(recipeDisplayEntry));
                continue;
            }
