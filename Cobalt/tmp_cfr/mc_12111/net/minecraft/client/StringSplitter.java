...
public class StringSplitter {
    ...
    public float stringWidth(@Nullable String string) {
        ...
        StringDecomposer.iterateFormatted((String)string, (Style)Style.EMPTY, (i, style, j) -> {
    ...
    public float stringWidth(FormattedText formattedText) {
        MutableFloat mutableFloat = new MutableFloat();
        StringDecomposer.iterateFormatted((FormattedText)formattedText, (Style)Style.EMPTY, (i, style, j) -> {
            mutableFloat.add(this.widthProvider.getWidth(j, style));
            return true;
        });
        return mutableFloat.floatValue();
    ...
    public int plainIndexAtWidth(String string, int i, Style style) {
        ...
        StringDecomposer.iterate((String)string, (Style)style, (FormattedCharSink)widthLimitedCharSink);
    ...
    public String plainTailByWidth(String string, int i, Style style2) {
        ...
        StringDecomposer.iterateBackwards((String)string, (Style)style2, (j, style, k) -> {
    ...
    public FormattedText headByWidth(FormattedText formattedText, int i, Style style) {
        ...
        return formattedText.visit((FormattedText.StyledContentConsumer)new FormattedText.StyledContentConsumer<FormattedText>(){
            ...
            public Optional<FormattedText> accept(Style style, String string) {
                ...
                if (!StringDecomposer.iterateFormatted((String)string, (Style)style, (FormattedCharSink)widthLimitedCharSink)) {
                    String string2 = string.substring(0, widthLimitedCharSink.getPosition());
                    if (!string2.isEmpty()) {
                        this.collector.append(FormattedText.of((String)string2, (Style)style));
                    }
                    return Optional.of(this.collector.getResultOrEmpty());
                }
                if (!string.isEmpty()) {
                    this.collector.append(FormattedText.of((String)string, (Style)style));
                }
    ...
    public int findLineBreak(String string, int i, Style style) {
        ...
        StringDecomposer.iterateFormatted((String)string, (Style)style, (FormattedCharSink)lineBreakFinder);
    ...
    public void splitLines(String string, int i, Style style, boolean bl, LinePosConsumer linePosConsumer) {
        ...
        while (j < k) {
            LineBreakFinder lineBreakFinder = new LineBreakFinder(i);
            boolean bl2 = StringDecomposer.iterateFormatted((String)string, (int)j, (Style)style2, (Style)style, (FormattedCharSink)lineBreakFinder);
            if (bl2) {
    ...
    public List<FormattedText> splitLines(String string, int i2, Style style2) {
        ...
        this.splitLines(string, i2, style2, false, (style, i, j) -> list.add(FormattedText.of((String)string.substring(i, j), (Style)style)));
        return list;
    }
    ...
    public void splitLines(FormattedText formattedText, int i, Style style2, BiConsumer<FormattedText, Boolean> biConsumer) {
        ...
        block0: while (bl) {
            ...
            for (LineComponent lineComponent : flatComponents.parts) {
                boolean bl4 = StringDecomposer.iterateFormatted((String)lineComponent.contents, (int)0, (Style)lineComponent.style, (Style)style2, (FormattedCharSink)lineBreakFinder);
                if (!bl4) {
            ...
            while (listIterator.hasNext()) {
                ...
                if (!bl) {
                    if (k > l) {
                        ...
                        if (!string2.isEmpty()) {
                            componentCollector.append(FormattedText.of((String)string2, (Style)lineComponent.style));
                        }
                        k += j;
                        bl = true;
                    }
