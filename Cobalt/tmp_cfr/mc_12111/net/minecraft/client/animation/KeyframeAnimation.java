...
public class KeyframeAnimation {
    ...
    record Entry(ModelPart part, AnimationChannel.Target target, Keyframe[] keyframes) {
        public void apply(float f, float g, Vector3f vector3f) {
            int i2 = Math.max(0, Mth.binarySearch((int)0, (int)this.keyframes.length, i -> f <= this.keyframes[i].timestamp()) - 1);
            ...
            float k = j != i2 ? Mth.clamp((float)(h / (keyframe2.timestamp() - keyframe.timestamp())), (float)0.0f, (float)1.0f) : 0.0f;
