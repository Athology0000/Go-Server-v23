...
public class ConditionBuilder {
    ...
    public final <T extends Comparable<T>> ConditionBuilder term(Property<T> property, T comparable, T ... comparables) {
        List<KeyValueCondition.Term> list = Stream.concat(Stream.of(comparable), Stream.of(comparables)).map(arg_0 -> property.getName(arg_0)).sorted().distinct().map(string -> new KeyValueCondition.Term((String)string, false)).toList();
