...
implements ItemSlotMouseAction {
    ...
    public boolean onMouseScrolled(double d, double e, int i, ItemStack itemStack) {
        ...
        int l;
        int k;
        int j = BundleItem.getNumberOfItemsToShow((ItemStack)itemStack);
        if (j == 0) {
            return false;
        }
        Vector2i vector2i = this.scrollWheelHandler.onMouseScroll(d, e);
        int n = k = vector2i.y == 0 ? -vector2i.x : vector2i.y;
        if (k != 0 && (l = BundleItem.getSelectedItem((ItemStack)itemStack)) != (m = ScrollWheelHandler.getNextScrollWheelSelection(k, l, j))) {
    ...
    private void toggleSelectedBundleItem(ItemStack itemStack, int i, int j) {
        if (this.minecraft.getConnection() != null && j < BundleItem.getNumberOfItemsToShow((ItemStack)itemStack)) {
            ClientPacketListener clientPacketListener = this.minecraft.getConnection();
            BundleItem.toggleSelectedItem((ItemStack)itemStack, (int)j);
            clientPacketListener.send((Packet<?>)new ServerboundSelectBundleItemPacket(i, j));
        }
    }
