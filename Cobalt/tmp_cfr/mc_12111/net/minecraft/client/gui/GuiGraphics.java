...
public class GuiGraphics {
    ...
    public void drawString(Font font, @Nullable String string, int i, int j, int k, boolean bl) {
        ...
        this.drawString(font, Language.getInstance().getVisualOrder(FormattedText.of((String)string)), i, j, k, bl);
    }
    ...
    public void drawStringWithBackdrop(Font font, Component component, int i, int j, int k, int l) {
        ...
        if (m != 0) {
            int n = 2;
            this.fill(i - 2, j - 2, i + k + 2, j + font.lineHeight + 2, ARGB.multiply((int)m, (int)l));
        }
    ...
    public void blitSprite(RenderPipeline renderPipeline, Identifier identifier, int i, int j, int k, int l, float f) {
        this.blitSprite(renderPipeline, identifier, i, j, k, l, ARGB.white((float)f));
    }
    ...
    public void renderItem(ItemStack itemStack, int i, int j) {
        this.renderItem((LivingEntity)this.minecraft.player, this.minecraft.level, itemStack, i, j, 0);
    }

    public void renderItem(ItemStack itemStack, int i, int j, int k) {
        this.renderItem((LivingEntity)this.minecraft.player, this.minecraft.level, itemStack, i, j, k);
    }
    ...
    private void renderItem(@Nullable LivingEntity livingEntity, @Nullable Level level, ItemStack itemStack, int i, int j, int k) {
        ...
        this.minecraft.getItemModelResolver().updateForTopItem(trackingItemStackRenderState, itemStack, ItemDisplayContext.GUI, level, (ItemOwner)livingEntity, k);
        try {
    ...
    public void setTooltipForNextFrame(Font font, ItemStack itemStack, int i, int j) {
        this.setTooltipForNextFrame(font, Screen.getTooltipFromItem(this.minecraft, itemStack), itemStack.getTooltipImage(), i, j, (Identifier)itemStack.get(DataComponents.TOOLTIP_STYLE));
    }
    ...
    public void setTooltipForNextFrame(Font font, List<Component> list, Optional<TooltipComponent> optional, int i, int j, @Nullable Identifier identifier) {
        List list2 = (List)list.stream().map(Component::getVisualOrderText).map(ClientTooltipComponent::create).collect(Util.toMutableList());
    ...
    private void renderItemBar(ItemStack itemStack, int i, int j) {
        if (itemStack.isBarVisible()) {
            ...
            this.fill(RenderPipelines.GUI, k, l, k + itemStack.getBarWidth(), l + 1, ARGB.opaque((int)itemStack.getBarColor()));
        }
    }
    ...
    private void renderItemCooldown(ItemStack itemStack, int i, int j) {
        ...
        if (f > 0.0f) {
            int k = j + Mth.floor((float)(16.0f * (1.0f - f)));
            int l = k + Mth.ceil((float)(16.0f * f));
        ...
        switch (SwitchBootstraps.typeSwitch("typeSwitch", new Object[]{HoverEvent.ShowItem.class, HoverEvent.ShowEntity.class, HoverEvent.ShowText.class}, (Object)hoverEvent2, n)) {
            ...
            case 2: {
                HoverEvent.ShowText showText = (HoverEvent.ShowText)hoverEvent2;
                {
                    ...
                    this.setTooltipForNextFrame(font, font.split((FormattedText)component2, Math.max(this.guiWidth() / 2, 200)), i, j);
                    return;
                }
            }
        }
    ...
    public void submitMapRenderState(MapRenderState mapRenderState) {
        ...
        for (MapRenderState.MapDecorationRenderState mapDecorationRenderState : mapRenderState.decorations) {
            ...
            float f = font.width((FormattedText)mapDecorationRenderState.name);
            float f2 = 25.0f / f;
            Objects.requireNonNull(font);
            float g = Mth.clamp((float)f2, (float)0.0f, (float)(6.0f / 9.0f));
            this.pose.pushMatrix();
    ...
    Consumer<Style> {
        ...
        public void accept(TextAlignment textAlignment, int i, int j, ActiveTextCollector.Parameters parameters, FormattedCharSequence formattedCharSequence) {
            ...
            GuiTextRenderState guiTextRenderState = new GuiTextRenderState(GuiGraphics.this.minecraft.font, formattedCharSequence, parameters.pose(), k, j, ARGB.white((float)parameters.opacity()), 0, true, bl, parameters.scissor());
        ...
        public void acceptScrolling(Component component, int i, int j, int k, int l, int m, ActiveTextCollector.Parameters parameters) {
            int n = GuiGraphics.this.minecraft.font.width((FormattedText)component);
