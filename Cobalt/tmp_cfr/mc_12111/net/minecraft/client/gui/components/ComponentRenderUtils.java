...
public class ComponentRenderUtils {
    ...
    private static String stripColor(String string) {
        return Minecraft.getInstance().options.chatColors().get() != false ? string : ChatFormatting.stripFormatting((String)string);
    }
    ...
    public static List<FormattedCharSequence> wrapComponents(FormattedText formattedText2, int i, Font font) {
        ...
        formattedText2.visit((style, string) -> {
            componentCollector.append(FormattedText.of((String)ComponentRenderUtils.stripColor(string), (Style)style));
        ...
        font.getSplitter().splitLines(componentCollector.getResultOrEmpty(), i, Style.EMPTY, (formattedText, boolean_) -> {
            ...
            list.add(boolean_ != false ? FormattedCharSequence.composite((FormattedCharSequence)INDENT, (FormattedCharSequence)formattedCharSequence) : formattedCharSequence);
        });
        if (list.isEmpty()) {
