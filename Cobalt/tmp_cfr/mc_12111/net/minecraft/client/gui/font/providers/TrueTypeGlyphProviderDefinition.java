...
public record TrueTypeGlyphProviderDefinition(Identifier location, float size, float oversample, Shift shift, String skip) implements GlyphProviderDefinition
{
    ...
    extends Record {
        ...
        public static final Codec<Shift> CODEC = Codec.floatRange((float)-512.0f, (float)512.0f).listOf().comapFlatMap(list2 -> Util.fixedSize((List)list2, (int)2).map(list -> new Shift(((Float)list.get(0)).floatValue(), ((Float)list.get(1)).floatValue())), shift -> List.of(Float.valueOf(shift.x), Float.valueOf(shift.y)));
