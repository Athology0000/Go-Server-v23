...
public class AdvancementWidget {
    ...
    public AdvancementWidget(AdvancementTab advancementTab, Minecraft minecraft, AdvancementNode advancementNode, DisplayInfo displayInfo) {
        ...
        this.titleLines = minecraft.font.split((FormattedText)displayInfo.getTitle(), 163);
        this.x = Mth.floor((float)(displayInfo.getX() * 28.0f));
        this.y = Mth.floor((float)(displayInfo.getY() * 27.0f));
        int i = Math.max(this.titleLines.stream().mapToInt(minecraft.font::width).max().orElse(0), 80);
        int j = this.getMaxProgressWidth();
        int k = 29 + i + j;
        this.description = Language.getInstance().getVisualOrder(this.findOptimalLines(ComponentUtils.mergeStyles((Component)displayInfo.getDescription(), (Style)Style.EMPTY.withColor(displayInfo.getType().getChatColor())), k));
    ...
    private int getMaxProgressWidth() {
        ...
        int j = 8;
        MutableComponent component = Component.translatable((String)"advancements.progress", (Object[])new Object[]{i, i});
    ...
    public void drawHover(GuiGraphics guiGraphics, int i, int j, float f, int k, int l) {
        ...
        int r = component == null ? 0 : font.width((FormattedText)component);
        boolean bl2 = o + q >= 113;
        float g = this.progress == null ? 0.0f : this.progress.getPercent();
        int s = Mth.floor((float)(g * (float)this.width));
        if (g >= 1.0f) {
            s = this.width / 2;
