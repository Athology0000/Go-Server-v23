...
extends Screen {
    static final Identifier SLOT_SPRITE = Identifier.withDefaultNamespace((String)"gamemode_switcher/slot");
    static final Identifier SELECTION_SPRITE = Identifier.withDefaultNamespace((String)"gamemode_switcher/selection");
    ...
    public void render(GuiGraphics guiGraphics, int i, int j, float f) {
        ...
        MutableComponent mutableComponent = Component.translatable((String)"debug.gamemodes.select_next", (Object[])new Object[]{this.minecraft.options.keyDebugSwitchGameMode.getTranslatedKeyMessage().copy().withStyle(ChatFormatting.AQUA)});
        guiGraphics.drawCenteredString(this.font, (Component)mutableComponent, this.width / 2, this.height / 2 + 5, -1);
    ...
    private static void switchToHoveredGameMode(Minecraft minecraft, GameModeIcon gameModeIcon) {
        ...
        if (gameModeIcon != gameModeIcon2 && GameModeCommand.PERMISSION_CHECK.check(minecraft.player.permissions())) {
            minecraft.player.connection.send((Packet<?>)new ServerboundChangeGameModePacket(gameModeIcon.mode));
        }
    }
