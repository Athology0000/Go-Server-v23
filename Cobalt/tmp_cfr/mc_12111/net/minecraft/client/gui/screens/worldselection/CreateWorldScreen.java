...
extends Screen {
    ...
    static final Component GAME_MODEL_LABEL = Component.translatable((String)"selectWorld.gameMode");
    static final Component NAME_LABEL = Component.translatable((String)"selectWorld.enterName");
    static final Component EXPERIMENTS_LABEL = Component.translatable((String)"selectWorld.experiments");
    static final Component ALLOW_COMMANDS_INFO = Component.translatable((String)"selectWorld.allowCommands.info");
    ...
    public static void openFresh(Minecraft minecraft, Runnable runnable) {
        CreateWorldScreen.openFresh(minecraft, runnable, (createWorldScreen, layeredRegistryAccess, primaryLevelData, path) -> createWorldScreen.createNewWorld((LayeredRegistryAccess<RegistryLayer>)layeredRegistryAccess, (WorldData)primaryLevelData));
    }

    public static void openFresh(Minecraft minecraft, Runnable runnable, CreateWorldCallback createWorldCallback) {
        WorldCreationContextMapper worldCreationContextMapper = (reloadableServerResources, layeredRegistryAccess, dataPackReloadCookie) -> new WorldCreationContext(dataPackReloadCookie.worldGenSettings(), (LayeredRegistryAccess<RegistryLayer>)layeredRegistryAccess, reloadableServerResources, dataPackReloadCookie.dataConfiguration());
        Function<WorldLoader.DataLoadContext, WorldGenSettings> function = dataLoadContext -> new WorldGenSettings(WorldOptions.defaultWithRandomSeed(), WorldPresets.createNormalWorldDimensions((HolderLookup.Provider)dataLoadContext.datapackWorldgen()));
        CreateWorldScreen.openCreateWorldScreen(minecraft, runnable, function, worldCreationContextMapper, (ResourceKey<WorldPreset>)WorldPresets.NORMAL, createWorldCallback);
    }

    public static void testWorld(Minecraft minecraft, Runnable runnable) {
        WorldCreationContextMapper worldCreationContextMapper = (reloadableServerResources, layeredRegistryAccess, dataPackReloadCookie) -> new WorldCreationContext(dataPackReloadCookie.worldGenSettings().options(), dataPackReloadCookie.worldGenSettings().dimensions(), (LayeredRegistryAccess<RegistryLayer>)layeredRegistryAccess, reloadableServerResources, dataPackReloadCookie.dataConfiguration(), new InitialWorldCreationOptions(WorldCreationUiState.SelectedGameMode.CREATIVE, new GameRuleMap.Builder().set(GameRules.ADVANCE_TIME, (Object)false).set(GameRules.ADVANCE_WEATHER, (Object)false).set(GameRules.SPAWN_MOBS, (Object)false).build(), (ResourceKey<FlatLevelGeneratorPreset>)FlatLevelGeneratorPresets.REDSTONE_READY));
        Function<WorldLoader.DataLoadContext, WorldGenSettings> function = dataLoadContext -> new WorldGenSettings(WorldOptions.testWorldWithRandomSeed(), WorldPresets.createFlatWorldDimensions((HolderLookup.Provider)dataLoadContext.datapackWorldgen()));
        CreateWorldScreen.openCreateWorldScreen(minecraft, runnable, function, worldCreationContextMapper, (ResourceKey<WorldPreset>)WorldPresets.FLAT, (createWorldScreen, layeredRegistryAccess, primaryLevelData, path) -> createWorldScreen.createNewWorld((LayeredRegistryAccess<RegistryLayer>)layeredRegistryAccess, (WorldData)primaryLevelData));
    }
    ...
    private static void openCreateWorldScreen(Minecraft minecraft, Runnable runnable, Function<WorldLoader.DataLoadContext, WorldGenSettings> function, WorldCreationContextMapper worldCreationContextMapper, ResourceKey<WorldPreset> resourceKey, CreateWorldCallback createWorldCallback) {
        ...
        CompletableFuture completableFuture = WorldLoader.load((WorldLoader.InitConfig)initConfig, dataLoadContext -> new WorldLoader.DataLoadOutput((Object)new DataPackReloadCookie((WorldGenSettings)function.apply(dataLoadContext), dataLoadContext.dataConfiguration()), dataLoadContext.datapackDimensions()), (closeableResourceManager, reloadableServerResources, layeredRegistryAccess, dataPackReloadCookie) -> {
            closeableResourceManager.close();
            return worldCreationContextMapper.apply(reloadableServerResources, (LayeredRegistryAccess<RegistryLayer>)layeredRegistryAccess, (DataPackReloadCookie)dataPackReloadCookie);
        }, (Executor)Util.backgroundExecutor(), (Executor)((Object)minecraft));
        minecraft.managedBlock(completableFuture::isDone);
        minecraft.setScreen(new CreateWorldScreen(minecraft, runnable, (WorldCreationContext)completableFuture.join(), Optional.of(resourceKey), OptionalLong.empty(), createWorldCallback));
    }
    ...
    public static CreateWorldScreen createFromExisting(Minecraft minecraft, Runnable runnable, LevelSettings levelSettings, WorldCreationContext worldCreationContext, @Nullable Path path2) {
        CreateWorldScreen createWorldScreen2 = new CreateWorldScreen(minecraft, runnable, worldCreationContext, WorldPresets.fromSettings((WorldDimensions)worldCreationContext.selectedDimensions()), OptionalLong.of(worldCreationContext.options().seed()), (createWorldScreen, layeredRegistryAccess, primaryLevelData, path) -> createWorldScreen.createNewWorld((LayeredRegistryAccess<RegistryLayer>)layeredRegistryAccess, (WorldData)primaryLevelData));
    ...
    protected void init() {
        ...
        linearLayout.addChild(Button.builder((Component)Component.translatable((String)"selectWorld.create"), button -> this.onCreate()).build());
    ...
    private void onCreate() {
        ...
        LayeredRegistryAccess layeredRegistryAccess = worldCreationContext.worldgenRegistries().replaceFrom((Object)RegistryLayer.DIMENSIONS, new RegistryAccess.Frozen[]{complete.dimensionsRegistryAccess()});
        Lifecycle lifecycle = FeatureFlags.isExperimental((FeatureFlagSet)worldCreationContext.dataConfiguration().enabledFeatures()) ? Lifecycle.experimental() : Lifecycle.stable();
        ...
        WorldOpenFlows.confirmWorldCreation(this.minecraft, this, lifecycle3, () -> this.createWorldAndCleanup((LayeredRegistryAccess<RegistryLayer>)layeredRegistryAccess, primaryLevelData), bl);
    }
    ...
    void openDataPackSelectionScreen(WorldDataConfiguration worldDataConfiguration) {
        Pair<Path, PackRepository> pair = this.getDataPackSelectionSettings(worldDataConfiguration);
        if (pair != null) {
            this.minecraft.setScreen(new PackSelectionScreen((PackRepository)pair.getSecond(), packRepository -> this.tryApplyNewDataPacks((PackRepository)packRepository, true, this::openDataPackSelectionScreen), (Path)pair.getFirst(), (Component)Component.translatable((String)"dataPack.title")));
        }
    }

    private void tryApplyNewDataPacks(PackRepository packRepository, boolean bl2, Consumer<WorldDataConfiguration> consumer) {
        List list2;
        ImmutableList list = ImmutableList.copyOf((Collection)packRepository.getSelectedIds());
        WorldDataConfiguration worldDataConfiguration = new WorldDataConfiguration(new DataPackConfig((List)list, list2 = (List)packRepository.getAvailableIds().stream().filter(arg_0 -> CreateWorldScreen.method_29983((List)list, arg_0)).collect(ImmutableList.toImmutableList())), this.uiState.getSettings().dataConfiguration().enabledFeatures());
        ...
        if (FeatureFlags.isExperimental((FeatureFlagSet)featureFlagSet) && bl2) {
    ...
    private void applyNewPackConfig(PackRepository packRepository, WorldDataConfiguration worldDataConfiguration, Consumer<WorldDataConfiguration> consumer) {
        ...
        WorldLoader.InitConfig initConfig = CreateWorldScreen.createDefaultLoadConfig(packRepository, worldDataConfiguration);
        ((CompletableFuture)((CompletableFuture)WorldLoader.load((WorldLoader.InitConfig)initConfig, dataLoadContext -> {
            ...
            WorldCreationContext worldCreationContext = this.uiState.getSettings();
            RegistryOps dynamicOps = worldCreationContext.worldgenLoadContext().createSerializationContext((DynamicOps)JsonOps.INSTANCE);
            DataResult dataResult = WorldGenSettings.encode((DynamicOps)dynamicOps, (WorldOptions)worldCreationContext.options(), (WorldDimensions)worldCreationContext.selectedDimensions()).setLifecycle(Lifecycle.stable());
            RegistryOps dynamicOps2 = dataLoadContext.datapackWorldgen().createSerializationContext((DynamicOps)JsonOps.INSTANCE);
            WorldGenSettings worldGenSettings = (WorldGenSettings)dataResult.flatMap(arg_0 -> CreateWorldScreen.method_45682((DynamicOps)dynamicOps2, arg_0)).getOrThrow(string -> new IllegalStateException("Error parsing worldgen settings after loading data packs: " + string));
            return new WorldLoader.DataLoadOutput((Object)new DataPackReloadCookie(worldGenSettings, dataLoadContext.dataConfiguration()), dataLoadContext.datapackDimensions());
        }, (closeableResourceManager, reloadableServerResources, layeredRegistryAccess, dataPackReloadCookie) -> {
            closeableResourceManager.close();
            return new WorldCreationContext(dataPackReloadCookie.worldGenSettings(), (LayeredRegistryAccess<RegistryLayer>)layeredRegistryAccess, reloadableServerResources, dataPackReloadCookie.dataConfiguration());
        }, (Executor)Util.backgroundExecutor(), (Executor)((Object)this.minecraft)).thenApply(worldCreationContext -> {
            worldCreationContext.validate();
            return worldCreationContext;
        })).thenAcceptAsync(this.uiState::setSettings, (Executor)((Object)this.minecraft))).handleAsync((void_, throwable) -> {
            if (throwable != null) {
    ...
    private static void copyBetweenDirs(Path path, Path path2, Path path3) {
        try {
            Util.copyBetweenDirs((Path)path, (Path)path2, (Path)path3);
        }
        catch (IOException iOException) {
    ...
    private static Optional<LevelStorageSource.LevelStorageAccess> createNewWorldDirectory(Minecraft minecraft, String string, @Nullable Path path) {
        ...
        block12: {
            ...
            try {
                ...
                FileUtil.createDirectoriesSafe((Path)path22);
    ...
    private @Nullable Pair<Path, PackRepository> getDataPackSelectionSettings(WorldDataConfiguration worldDataConfiguration) {
        ...
        if (path != null) {
            if (this.tempDataPackRepository == null) {
                this.tempDataPackRepository = ServerPacksSource.createPackRepository((Path)path, (DirectoryValidator)this.packValidator);
                this.tempDataPackRepository.reload();
            }
            this.tempDataPackRepository.setSelected((Collection)worldDataConfiguration.dataPacks().getEnabled());
            return Pair.of((Object)path, (Object)this.tempDataPackRepository);
        }
        return null;
    }

    private static /* synthetic */ DataResult method_45682(DynamicOps dynamicOps, JsonElement jsonElement) {
        return WorldGenSettings.CODEC.parse(dynamicOps, (Object)jsonElement);
    }
    ...
    extends GridLayoutTab {
        ...
        GameTab() {
            ...
            this.nameEdit = new EditBox(CreateWorldScreen.this.font, 208, 20, (Component)Component.translatable((String)"selectWorld.enterName"));
            ...
            CreateWorldScreen.this.uiState.addListener(worldCreationUiState -> this.nameEdit.setTooltip(Tooltip.create((Component)Component.translatable((String)"selectWorld.targetFolder", (Object[])new Object[]{Component.literal((String)worldCreationUiState.getTargetFolder()).withStyle(ChatFormatting.ITALIC)}))));
            ...
            CycleButton<Difficulty> cycleButton22 = rowHelper.addChild(CycleButton.builder(Difficulty::getDisplayName, CreateWorldScreen.this.uiState.getDifficulty()).withValues((Difficulty[])Difficulty.values()).create(0, 0, 210, 20, (Component)Component.translatable((String)"options.difficulty"), (cycleButton, difficulty) -> CreateWorldScreen.this.uiState.setDifficulty((Difficulty)difficulty)), layoutSettings);
            CreateWorldScreen.this.uiState.addListener(worldCreationUiState -> {
    ...
    extends GridLayoutTab {
        ...
        static final Component SEED_EMPTY_HINT = Component.translatable((String)"selectWorld.seedInfo");
        ...
        WorldTab() {
            ...
            this.customizeTypeButton = rowHelper.addChild(Button.builder((Component)Component.translatable((String)"selectWorld.customizeType"), button -> this.openPresetEditor()).build());
            CreateWorldScreen.this.uiState.addListener(worldCreationUiState -> {
                this.customizeTypeButton.active = !worldCreationUiState.isDebug() && worldCreationUiState.getPresetEditor() != null;
            });
            this.seedEdit = new EditBox(this, CreateWorldScreen.this.font, 308, 20, (Component)Component.translatable((String)"selectWorld.enterSeed")){
        ...
        private static MutableComponent createTypeButtonNarration(CycleButton<WorldCreationUiState.WorldTypeEntry> cycleButton) {
            if (cycleButton.getValue().isAmplified()) {
                return CommonComponents.joinForNarration((Component[])new Component[]{cycleButton.createDefaultNarrationMessage(), AMPLIFIED_HELP_TEXT});
            }
