...
public class AnimationUtils {
    ...
    public static void animateCrossbowCharge(ModelPart modelPart, ModelPart modelPart2, float f, float g, boolean bl) {
        ...
        float h = Mth.clamp((float)g, (float)0.0f, (float)f);
        float i = h / f;
        modelPart4.yRot = Mth.lerp((float)i, (float)0.4f, (float)0.85f) * (float)(bl ? 1 : -1);
        modelPart4.xRot = Mth.lerp((float)i, (float)modelPart4.xRot, (float)-1.5707964f);
    }

    public static void swingWeaponDown(ModelPart modelPart, ModelPart modelPart2, HumanoidArm humanoidArm, float f, float g) {
        float h = Mth.sin((double)(f * (float)Math.PI));
        float i = Mth.sin((double)((1.0f - (1.0f - f) * (1.0f - f)) * (float)Math.PI));
        modelPart.zRot = 0.0f;
        modelPart2.zRot = 0.0f;
        modelPart.yRot = 0.15707964f;
        modelPart2.yRot = -0.15707964f;
        if (humanoidArm == HumanoidArm.RIGHT) {
            modelPart.xRot = -1.8849558f + Mth.cos((double)(g * 0.09f)) * 0.15f;
            modelPart2.xRot = -0.0f + Mth.cos((double)(g * 0.19f)) * 0.5f;
            modelPart.xRot += h * 2.2f - i * 0.4f;
            modelPart2.xRot += h * 1.2f - i * 0.4f;
        } else {
            modelPart.xRot = -0.0f + Mth.cos((double)(g * 0.19f)) * 0.5f;
            modelPart2.xRot = -1.8849558f + Mth.cos((double)(g * 0.09f)) * 0.15f;
    ...
    public static void bobModelPart(ModelPart modelPart, float f, float g) {
        modelPart.zRot += g * (Mth.cos((double)(f * 0.09f)) * 0.05f + 0.05f);
        modelPart.xRot += g * (Mth.sin((double)(f * 0.067f)) * 0.05f);
    }
