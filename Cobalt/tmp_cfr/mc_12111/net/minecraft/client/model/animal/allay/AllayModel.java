...
implements ArmedModel<AllayRenderState> {
    ...
    public void setupAnim(AllayRenderState allayRenderState) {
        ...
        float i = Mth.cos((double)h) * (float)Math.PI * 0.15f + f;
        ...
        if (allayRenderState.isDancing) {
            ...
            o = Mth.cos((double)n) * 16.0f * ((float)Math.PI / 180);
            p = allayRenderState.spinningProgress;
            float q = Mth.cos((double)n) * 14.0f * ((float)Math.PI / 180);
            float r = Mth.cos((double)n) * 30.0f * ((float)Math.PI / 180);
        ...
        n = m * Mth.lerp((float)k, (float)-1.0471976f, (float)-1.134464f);
        this.root.y += (float)Math.cos(j) * 0.25f * l;
        this.right_arm.xRot = n;
        this.left_arm.xRot = n;
        o = l * (1.0f - m);
        p = 0.43633232f - Mth.cos((double)(j + 4.712389f)) * (float)Math.PI * 0.075f * o;
