...
extends EntityModel<GuardianRenderState> {
    ...
    public void setupAnim(GuardianRenderState guardianRenderState) {
        ...
        this.tailParts[0].yRot = Mth.sin((double)g) * (float)Math.PI * 0.05f;
        this.tailParts[1].yRot = Mth.sin((double)g) * (float)Math.PI * 0.1f;
        this.tailParts[2].yRot = Mth.sin((double)g) * (float)Math.PI * 0.15f;
    }
    ...
    private static float getSpikeOffset(int i, float f, float g) {
        return 1.0f + Mth.cos((double)(f * 1.5f + (float)i)) * 0.01f - g;
    }
