...
extends EntityModel<HoglinRenderState> {
    ...
    public void setupAnim(HoglinRenderState hoglinRenderState) {
        ...
        this.rightEar.zRot = -0.6981317f - f * Mth.sin((double)g);
        this.leftEar.zRot = 0.6981317f + f * Mth.sin((double)g);
        this.head.yRot = hoglinRenderState.yRot * ((float)Math.PI / 180);
        float h = 1.0f - (float)Mth.abs((int)(10 - 2 * hoglinRenderState.attackAnimationRemainingTicks)) / 10.0f;
        this.head.xRot = Mth.lerp((float)h, (float)0.87266463f, (float)-0.34906584f);
        if (hoglinRenderState.isBaby) {
            this.head.y += h * 2.5f;
        }
        float i = 1.2f;
        this.rightFrontLeg.xRot = Mth.cos((double)g) * 1.2f * f;
        this.rightHindLeg.xRot = this.leftFrontLeg.xRot = Mth.cos((double)(g + (float)Math.PI)) * 1.2f * f;
