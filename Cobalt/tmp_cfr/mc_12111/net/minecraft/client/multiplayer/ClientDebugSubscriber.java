...
public class ClientDebugSubscriber {
    ...
    private void onSubscriptionsChanged(Set<DebugSubscription<?>> set) {
        ...
        this.connection.send((Packet<?>)new ServerboundDebugSubscriptionRequestPacket(set));
    }
    ...
    public <T> void pushEvent(long l, DebugSubscription.Event<T> event) {
        ValueMaps valueMaps = this.getValueMaps(event.subscription());
        if (valueMaps != null) {
            valueMaps.events.add(new ValueWrapper<Object>(event.value(), l + (long)event.subscription().expireAfterTicks()));
        }
    }
    ...
    static class ValueMap<K, V> {
        ...
        public void apply(long l, K object, DebugSubscription.Update<V> update) {
            if (update.value().isPresent()) {
                this.values.put(object, new ValueWrapper(update.value().get(), l + (long)update.subscription().expireAfterTicks()));
            } else {
                this.values.remove(object);
            }
        }
