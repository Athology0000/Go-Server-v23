...
public class SessionSearchTrees {
    ...
    private static Stream<String> getTooltipLines(Stream<ItemStack> stream, Item.TooltipContext tooltipContext, TooltipFlag tooltipFlag) {
        return stream.flatMap(itemStack -> itemStack.getTooltipLines(tooltipContext, null, tooltipFlag).stream()).map(component -> ChatFormatting.stripFormatting((String)component.getString()).trim()).filter(string -> !string.isEmpty());
    }
    ...
    public void updateRecipes(ClientRecipeBook clientRecipeBook, Level level) {
        this.register(RECIPE_COLLECTIONS, () -> {
            ...
            Registry registry = registryAccess.lookupOrThrow(Registries.ITEM);
            ...
            this.recipeSearch = CompletableFuture.supplyAsync(() -> SessionSearchTrees.method_60361(contextMap, tooltipContext, (TooltipFlag)tooltipFlag, registry, list), (Executor)Util.backgroundExecutor());
    ...
    public void updateCreativeTags(List<ItemStack> list) {
        this.register(CREATIVE_TAGS, () -> {
            ...
            this.creativeByTagSearch = CompletableFuture.supplyAsync(() -> new IdSearchTree<ItemStack>(itemStack -> itemStack.getTags().map(TagKey::location), list), (Executor)Util.backgroundExecutor());
            completableFuture.cancel(true);
        });
    }
    ...
    public void updateCreativeTooltips(HolderLookup.Provider provider, List<ItemStack> list) {
        this.register(CREATIVE_NAMES, () -> {
            Item.TooltipContext tooltipContext = Item.TooltipContext.of((HolderLookup.Provider)provider);
            ...
            this.creativeByNameSearch = CompletableFuture.supplyAsync(() -> SessionSearchTrees.method_60350(tooltipContext, (TooltipFlag)tooltipFlag, list), (Executor)Util.backgroundExecutor());
            completableFuture.cancel(true);
        });
    }
    ...
    private static /* synthetic */ SearchTree method_60350(Item.TooltipContext tooltipContext, TooltipFlag tooltipFlag, List list) {
        return new FullTextSearchTree<ItemStack>(itemStack -> SessionSearchTrees.getTooltipLines(Stream.of(itemStack), tooltipContext, tooltipFlag), itemStack -> itemStack.getItemHolder().unwrapKey().map(ResourceKey::identifier).stream(), list);
    }
