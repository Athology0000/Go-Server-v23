...
extends AbstractClientPlayer {
    ...
    public boolean startRiding(Entity entity, boolean bl, boolean bl2) {
        ...
        if (entity instanceof AbstractMinecart) {
            ...
            this.minecraft.getSoundManager().play(new RidingEntitySoundInstance(this, (Entity)abstractNautilus, true, SoundEvents.NAUTILUS_RIDING, abstractNautilus.getSoundSource(), 0.0f, 1.0f, 5.0f));
        }
        return true;
    }
    ...
    private void sendPosition() {
        ...
        if (this.isControlledCamera()) {
            ...
            boolean bl = Mth.lengthSquared((double)d, (double)e, (double)f) > Mth.square((double)2.0E-4) || this.positionReminder >= 20;
    ...
    public void removeRecipeHighlight(RecipeDisplayId recipeDisplayId) {
        if (this.recipeBook.willHighlight(recipeDisplayId)) {
            ...
            this.connection.send((Packet<?>)new ServerboundRecipeBookSeenRecipePacket(recipeDisplayId));
        }
    }
    ...
    private void moveTowardsClosestSpace(double d, double e) {
        ...
        BlockPos blockPos = BlockPos.containing((double)d, (double)this.getY(), (double)e);
    ...
    private boolean suffocatesAt(BlockPos blockPos) {
        ...
        return this.level().collidesWithSuffocatingBlock((Entity)this, aABB2);
    }
    ...
    public void openTextEdit(SignBlockEntity signBlockEntity, boolean bl) {
        if (signBlockEntity instanceof HangingSignBlockEntity) {
            ...
            this.minecraft.setScreen(new HangingSignEditScreen((SignBlockEntity)hangingSignBlockEntity, bl, this.minecraft.isTextFilteringEnabled()));
        } else {
    ...
    private static float distanceToUnitSquare(Vec2 vec2) {
        ...
        float h = g > f ? f / g : g / f;
        return Mth.sqrt((float)(1.0f + Mth.square((float)h)));
    }
    ...
    private void handlePortalTransitionEffect(boolean bl) {
        ...
        this.portalEffectIntensity = Mth.clamp((float)(this.portalEffectIntensity + f), (float)0.0f, (float)1.0f);
    }
    ...
    public void move(MoverType moverType, Vec3 vec3) {
        ...
        this.addWalkedDistance(Mth.length((float)f, (float)g) * 0.6f);
    }
    ...
    protected void updateAutoJump(float f, float g) {
        ...
        if (i <= 0.001f) {
            ...
            float j = h * vec2.x;
            float k = h * vec2.y;
            l = Mth.sin((double)(this.getYRot() * ((float)Math.PI / 180)));
            float m = Mth.cos((double)(this.getYRot() * ((float)Math.PI / 180)));
        ...
        if (!blockState.getCollisionShape((BlockGetter)this.level(), blockPos, collisionContext).isEmpty()) {
            return;
        }
        blockPos = blockPos.above();
        BlockState blockState2 = this.level().getBlockState(blockPos);
        if (!blockState2.getCollisionShape((BlockGetter)this.level(), blockPos, collisionContext).isEmpty()) {
            return;
        }
        float o = 7.0f;
        float p = 1.2f;
        ...
        AABB aABB = new AABB(vec36, vec37.add(0.0, (double)s, 0.0)).inflate((double)r, 0.0, (double)r);
        ...
        Iterable iterable = this.level().getCollisions((Entity)this, aABB);
        ...
        while (iterator.hasNext()) {
            ...
            BlockPos blockPos2 = BlockPos.containing((Position)vec314);
            int u = 1;
            while ((float)u < p) {
                BlockPos blockPos3 = blockPos2.above(u);
                BlockState blockState3 = this.level().getBlockState(blockPos3);
                VoxelShape voxelShape2 = blockState3.getCollisionShape((BlockGetter)this.level(), blockPos3, collisionContext);
                ...
                if (u > 1) {
                    ...
                    if (!blockState4.getCollisionShape((BlockGetter)this.level(), blockPos, collisionContext).isEmpty()) {
                        return;
                    }
                }
                ++u;
    ...
    protected boolean isHorizontalCollisionMinor(Vec3 vec3) {
        ...
        double d = Mth.sin((double)f);
        double e = Mth.cos((double)f);
        double g = (double)this.xxa * e - (double)this.zza * d;
        double h = (double)this.zza * e + (double)this.xxa * d;
        double i = Mth.square((double)g) + Mth.square((double)h);
        double j = Mth.square((double)vec3.x) + Mth.square((double)vec3.z);
    ...
    public float getWaterVision() {
        ...
        float h = Mth.clamp((float)((float)this.waterVisionTime / 100.0f), (float)0.0f, (float)1.0f);
        float i = (float)this.waterVisionTime < 100.0f ? 0.0f : Mth.clamp((float)(((float)this.waterVisionTime - 100.0f) / 500.0f), (float)0.0f, (float)1.0f);
    ...
    public Vec3 getRopeHoldPosition(float f) {
        if (this.minecraft.options.getCameraType().isFirstPerson()) {
            float g = Mth.lerp((float)(f * 0.5f), (float)this.getYRot(), (float)this.yRotO) * ((float)Math.PI / 180);
            float h = Mth.lerp((float)(f * 0.5f), (float)this.getXRot(), (float)this.xRotO) * ((float)Math.PI / 180);
    ...
    private static HitResult pick(Entity entity, double d, double e, float f) {
        ...
        EntityHitResult entityHitResult = ProjectileUtil.getEntityHitResult((Entity)entity, (Vec3)vec3, (Vec3)vec33, (AABB)aABB, (Predicate)EntitySelector.CAN_BE_PICKED, (double)h);
        if (entityHitResult != null && entityHitResult.getLocation().distanceToSqr(vec3) < i) {
            return LocalPlayer.filterHitResult((HitResult)entityHitResult, vec3, e);
        }
    ...
    private static HitResult filterHitResult(HitResult hitResult, Vec3 vec3, double d) {
        ...
        if (!vec32.closerThan((Position)vec3, d)) {
            Vec3 vec33 = hitResult.getLocation();
            Direction direction = Direction.getApproximateNearest((double)(vec33.x - vec3.x), (double)(vec33.y - vec3.y), (double)(vec33.z - vec3.z));
            return BlockHitResult.miss((Vec3)vec33, (Direction)direction, (BlockPos)BlockPos.containing((Position)vec33));
        }
        return hitResult;
    }
}
