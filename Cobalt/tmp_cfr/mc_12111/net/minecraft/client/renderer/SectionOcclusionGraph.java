...
public class SectionOcclusionGraph {
    ...
    private void addNeighbors(GraphEvents graphEvents, ChunkPos chunkPos) {
        graphEvents.chunksWhichReceivedNeighbors.add(ChunkPos.asLong((int)(chunkPos.x - 1), (int)chunkPos.z));
        graphEvents.chunksWhichReceivedNeighbors.add(ChunkPos.asLong((int)chunkPos.x, (int)(chunkPos.z - 1)));
        graphEvents.chunksWhichReceivedNeighbors.add(ChunkPos.asLong((int)(chunkPos.x + 1), (int)chunkPos.z));
        graphEvents.chunksWhichReceivedNeighbors.add(ChunkPos.asLong((int)chunkPos.x, (int)(chunkPos.z + 1)));
        graphEvents.chunksWhichReceivedNeighbors.add(ChunkPos.asLong((int)(chunkPos.x - 1), (int)(chunkPos.z - 1)));
        graphEvents.chunksWhichReceivedNeighbors.add(ChunkPos.asLong((int)(chunkPos.x - 1), (int)(chunkPos.z + 1)));
        graphEvents.chunksWhichReceivedNeighbors.add(ChunkPos.asLong((int)(chunkPos.x + 1), (int)(chunkPos.z - 1)));
        graphEvents.chunksWhichReceivedNeighbors.add(ChunkPos.asLong((int)(chunkPos.x + 1), (int)(chunkPos.z + 1)));
    }
    ...
    private void initializeQueueForFullUpdate(Camera camera, Queue<Node> queue) {
        ...
        long l = SectionPos.asLong((BlockPos)blockPos);
        ...
        if (renderSection == null) {
            ...
            int m = SectionPos.x((long)l);
            int n = SectionPos.z((long)l);
            for (int o = -k; o <= k; ++o) {
                for (int p = -k; p <= k; ++p) {
                    SectionRenderDispatcher.RenderSection renderSection2 = this.viewArea.getRenderSection(SectionPos.asLong((int)(o + m), (int)j, (int)(p + n)));
            ...
            list.sort(Comparator.comparingDouble(node -> blockPos.distSqr((Vec3i)SectionPos.of((long)node.section.getSectionNode()).center())));
            queue.addAll(list);
        } else {
    ...
    private void runUpdates(GraphStorage graphStorage, Vec3 vec3, Queue<Node> queue, boolean bl, Consumer<SectionRenderDispatcher.RenderSection> consumer, LongOpenHashSet longOpenHashSet) {
        SectionPos sectionPos = SectionPos.of((Position)vec3);
        ...
        while (!queue.isEmpty()) {
            ...
            boolean bl2 = Math.abs(SectionPos.x((long)(m = renderSection.getSectionNode())) - sectionPos.x()) > MINIMUM_ADVANCED_CULLING_SECTION_DISTANCE || Math.abs(SectionPos.y((long)m) - sectionPos.y()) > MINIMUM_ADVANCED_CULLING_SECTION_DISTANCE || Math.abs(SectionPos.z((long)m) - sectionPos.z()) > MINIMUM_ADVANCED_CULLING_SECTION_DISTANCE;
            for (Direction direction : DIRECTIONS) {
                ...
                if (bl && bl2) {
                    boolean bl5;
                    boolean bl4;
                    int j = SectionPos.sectionToBlockCoord((int)SectionPos.x((long)m));
                    int k = SectionPos.sectionToBlockCoord((int)SectionPos.y((long)m));
                    i = SectionPos.sectionToBlockCoord((int)SectionPos.z((long)m));
                    ...
                    while (vector3d.distanceSquared(vec3.x, vec3.y, vec3.z) > 3600.0) {
                        ...
                        SectionRenderDispatcher.RenderSection renderSection3 = this.viewArea.getRenderSectionAt(BlockPos.containing((double)vector3d.x, (double)vector3d.y, (double)vector3d.z));
                ...
                graphStorage.sectionToNodeMap.put(renderSection2, node3);
                long n = SectionPos.sectionToChunk((long)renderSection2.getSectionNode());
    ...
    private boolean isInViewDistance(long l, long m) {
        return ChunkTrackingView.isInViewDistance((int)SectionPos.x((long)l), (int)SectionPos.z((long)l), (int)this.viewArea.getViewDistance(), (int)SectionPos.x((long)m), (int)SectionPos.z((long)m));
    }
    ...
    private @Nullable SectionRenderDispatcher.RenderSection getRelativeFrom(long l, SectionRenderDispatcher.RenderSection renderSection, Direction direction) {
        ...
        if (Mth.abs((int)(SectionPos.y((long)l) - SectionPos.y((long)m))) > this.viewArea.getViewDistance()) {
            return null;
        }
