...
public interface BlockStateModel {
    ...
    extends ResolvableModel {
        ...
        public static final Codec<WeightedVariants.Unbaked> HARDCODED_WEIGHTED_CODEC = ExtraCodecs.nonEmptyList((Codec)ELEMENT_CODEC.listOf()).flatComapMap(list -> new WeightedVariants.Unbaked((WeightedList<Unbaked>)WeightedList.of((List)Lists.transform((List)list, weighted -> weighted.map(SingleVariant.Unbaked::new)))), unbaked -> {
            List list = unbaked.entries().unwrap();
            ArrayList<Weighted> list2 = new ArrayList<Weighted>(list.size());
            for (Weighted weighted : list) {
                ...
                if (object instanceof SingleVariant.Unbaked) {
                    ...
                    list2.add(new Weighted((Object)unbaked2.variant(), weighted.weight()));
                    continue;
                }
        ...
        public static final Codec<Unbaked> CODEC = Codec.either(HARDCODED_WEIGHTED_CODEC, SingleVariant.Unbaked.CODEC).flatComapMap(either -> (Unbaked)either.map(unbaked -> unbaked, unbaked -> unbaked), unbaked -> {
            ...
            int i = 0;
            return switch (SwitchBootstraps.typeSwitch("typeSwitch", new Object[]{SingleVariant.Unbaked.class, WeightedVariants.Unbaked.class}, (Object)unbaked22, i)) {
                case 0 -> {
