...
public interface Condition {
    public static final Codec<Condition> CODEC = Codec.recursive((String)"condition", codec -> {
        Codec codec2 = Codec.simpleMap(CombinedCondition.Operation.CODEC, (Codec)codec.listOf(), (Keyable)StringRepresentable.keys((StringRepresentable[])CombinedCondition.Operation.values())).codec().comapFlatMap(map -> {
            if (map.size() != 1) {
        ...
        return Codec.either((Codec)codec2, KeyValueCondition.CODEC).flatComapMap(either -> (Condition)either.map(combinedCondition -> combinedCondition, keyValueCondition -> keyValueCondition), condition -> {
            ...
            int i = 0;
            DataResult dataResult = switch (SwitchBootstraps.typeSwitch("typeSwitch", new Object[]{CombinedCondition.class, KeyValueCondition.class}, (Object)condition22, i)) {
                case 0 -> {
