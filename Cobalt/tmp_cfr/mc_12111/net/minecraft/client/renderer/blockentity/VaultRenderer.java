...
implements BlockEntityRenderer<VaultBlockEntity, VaultRenderState> {
    ...
    public void extractRenderState(VaultBlockEntity vaultBlockEntity, VaultRenderState vaultRenderState, float f, Vec3 vec3,  @Nullable ModelFeatureRenderer.CrumblingOverlay crumblingOverlay) {
        ...
        if (!VaultBlockEntity.Client.shouldDisplayActiveEffects((VaultSharedData)vaultBlockEntity.getSharedData()) || itemStack.isEmpty() || vaultBlockEntity.getLevel() == null) {
            return;
        }
        ...
        vaultRenderState.spin = Mth.rotLerp((float)f, (float)vaultClientData.previousSpin(), (float)vaultClientData.currentSpin());
    }
