...
extends EntityRenderer<AbstractBoat, BoatRenderState> {
    ...
    public void submit(BoatRenderState boatRenderState, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, CameraRenderState cameraRenderState) {
        ...
        if (f > 0.0f) {
            poseStack.mulPose((Quaternionfc)Axis.XP.rotationDegrees(Mth.sin((double)f) * f * boatRenderState.damageTime / 10.0f * (float)boatRenderState.hurtDir));
        }
        if (!boatRenderState.isUnderWater && !Mth.equal((float)boatRenderState.bubbleAngle, (float)0.0f)) {
