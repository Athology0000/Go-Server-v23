...
extends EntityRenderer<T, S> {
    private static final Identifier MINECART_LOCATION = Identifier.withDefaultNamespace((String)"textures/entity/minecart.png");
    ...
    public void submit(S minecartRenderState, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, CameraRenderState cameraRenderState) {
        ...
        if (i > 0.0f) {
            poseStack.mulPose((Quaternionfc)Axis.XP.rotationDegrees(Mth.sin((double)i) * i * ((MinecartRenderState)minecartRenderState).damageTime / 10.0f * (float)((MinecartRenderState)minecartRenderState).hurtDir));
        }
    ...
    public void extractRenderState(T abstractMinecart, S minecartRenderState, float f) {
        ...
        if (minecartBehavior instanceof NewMinecartBehavior) {
            ...
            minecartBehavior = abstractMinecart.getBehavior();
        ...
        ((MinecartRenderState)minecartRenderState).hurtTime = (float)abstractMinecart.getHurtTime() - f;
        ((MinecartRenderState)minecartRenderState).hurtDir = abstractMinecart.getHurtDir();
        ((MinecartRenderState)minecartRenderState).damageTime = Math.max(abstractMinecart.getDamage() - f, 0.0f);
        ((MinecartRenderState)minecartRenderState).displayOffset = abstractMinecart.getDisplayOffset();
        ((MinecartRenderState)minecartRenderState).displayBlockState = abstractMinecart.getDisplayBlockState();
    }
    ...
    private static <T extends AbstractMinecart, S extends MinecartRenderState> void oldExtractState(T abstractMinecart, OldMinecartBehavior oldMinecartBehavior, S minecartRenderState, float f) {
        ...
        if (vec3 != null) {
            ...
            minecartRenderState.frontPos = Objects.requireNonNullElse(vec32, vec3);
            minecartRenderState.backPos = Objects.requireNonNullElse(vec33, vec3);
        } else {
    ...
    protected AABB getBoundingBoxForCulling(T abstractMinecart) {
        ...
        if (!abstractMinecart.getDisplayBlockState().isAir()) {
            return aABB.expandTowards(0.0, (double)((float)abstractMinecart.getDisplayOffset() * 0.75f / 16.0f), 0.0);
        }
        return aABB;
    }
