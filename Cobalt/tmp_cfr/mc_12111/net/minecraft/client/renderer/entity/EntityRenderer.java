...
public abstract class EntityRenderer<T extends Entity, S extends EntityRenderState> {
    ...
    public final int getPackedLightCoords(T entity, float f) {
        BlockPos blockPos = BlockPos.containing((Position)entity.getLightProbePosition(f));
    ...
    public void extractRenderState(T entity, S entityRenderState, float f) {
        ...
        Entity entity3;
        ((EntityRenderState)entityRenderState).entityType = entity.getType();
        ((EntityRenderState)entityRenderState).x = Mth.lerp((double)f, (double)((Entity)entity).xOld, (double)entity.getX());
        ((EntityRenderState)entityRenderState).y = Mth.lerp((double)f, (double)((Entity)entity).yOld, (double)entity.getY());
        ((EntityRenderState)entityRenderState).z = Mth.lerp((double)f, (double)((Entity)entity).zOld, (double)entity.getZ());
        ((EntityRenderState)entityRenderState).isInvisible = entity.isInvisible();
        ((EntityRenderState)entityRenderState).ageInTicks = (float)((Entity)entity).tickCount + f;
        ((EntityRenderState)entityRenderState).boundingBoxWidth = entity.getBbWidth();
        ((EntityRenderState)entityRenderState).boundingBoxHeight = entity.getBbHeight();
        ((EntityRenderState)entityRenderState).eyeHeight = entity.getEyeHeight();
        if (entity.isPassenger() && (entity3 = entity.getVehicle()) instanceof AbstractMinecart && (entity3 = (abstractMinecart = (AbstractMinecart)entity3).getBehavior()) instanceof NewMinecartBehavior && (newMinecartBehavior = (NewMinecartBehavior)entity3).cartHasPosRotLerp()) {
            double d = Mth.lerp((double)f, (double)abstractMinecart.xOld, (double)abstractMinecart.getX());
            double e = Mth.lerp((double)f, (double)abstractMinecart.yOld, (double)abstractMinecart.getY());
            double g = Mth.lerp((double)f, (double)abstractMinecart.zOld, (double)abstractMinecart.getZ());
        ...
        if (this.entityRenderDispatcher.camera != null) {
            ...
            if (bl) {
                ...
                ((EntityRenderState)entityRenderState).nameTagAttachment = entity.getAttachments().getNullable(EntityAttachment.NAME_TAG, 0, entity.getYRot(f));
            } else {
                ((EntityRenderState)entityRenderState).nameTag = null;
            }
        }
        ((EntityRenderState)entityRenderState).isDiscrete = entity.isDiscrete();
        ...
        if (entity instanceof Leashable && (entity2 = (leashable = (Leashable)entity).getLeashHolder()) instanceof Entity) {
            ...
            float h = entity.getPreciseBodyRotation(f) * ((float)Math.PI / 180);
            Vec3 vec3 = leashable.getLeashOffset(f);
            BlockPos blockPos = BlockPos.containing((Position)entity.getEyePosition(f));
            BlockPos blockPos2 = BlockPos.containing((Position)entity22.getEyePosition(f));
            ...
            if (bl2) {
                ...
                EntityRenderState.LeashState leashState2 = ((EntityRenderState)entityRenderState).leashStates.getFirst();
        ...
        ((EntityRenderState)entityRenderState).displayFireAnimation = entity.displayFireAnimation();
        Minecraft minecraft = Minecraft.getInstance();
        boolean bl3 = minecraft.shouldEntityAppearGlowing((Entity)entity);
        ((EntityRenderState)entityRenderState).outlineColor = bl3 ? ARGB.opaque((int)entity.getTeamColor()) : 0;
    ...
    private void extractShadow(S entityRenderState, Minecraft minecraft, Level level) {
        ...
        if (minecraft.options.entityShadows().get().booleanValue() && !((EntityRenderState)entityRenderState).isInvisible) {
            ...
            if (f > 0.0f && (g = (float)((1.0 - (d = ((EntityRenderState)entityRenderState).distanceToCameraSq) / 256.0) * (double)this.getShadowStrength(entityRenderState))) > 0.0f) {
                int i = Mth.floor((double)(((EntityRenderState)entityRenderState).x - (double)f));
                int j = Mth.floor((double)(((EntityRenderState)entityRenderState).x + (double)f));
                int k = Mth.floor((double)(((EntityRenderState)entityRenderState).z - (double)f));
                int l = Mth.floor((double)(((EntityRenderState)entityRenderState).z + (double)f));
                float h = Math.min(g / 0.5f - 1.0f, f);
                int m = Mth.floor((double)(((EntityRenderState)entityRenderState).y - (double)h));
                int n = Mth.floor((double)((EntityRenderState)entityRenderState).y);
    ...
    private void extractShadowPiece(S entityRenderState, Level level, float f, BlockPos.MutableBlockPos mutableBlockPos, ChunkAccess chunkAccess) {
        ...
        int i = level.getMaxLocalRawBrightness((BlockPos)mutableBlockPos);
        if (i <= 3) {
            return;
        }
        if (!blockState.isCollisionShapeFullBlock((BlockGetter)chunkAccess, blockPos)) {
            return;
        }
        VoxelShape voxelShape = blockState.getShape((BlockGetter)chunkAccess, blockPos);
        if (voxelShape.isEmpty()) {
            return;
        }
        float h = Mth.clamp((float)(g * 0.5f * LightTexture.getBrightness(level.dimensionType(), i)), (float)0.0f, (float)1.0f);
