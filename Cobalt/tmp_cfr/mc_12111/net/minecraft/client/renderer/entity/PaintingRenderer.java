...
extends EntityRenderer<Painting, PaintingRenderState> {
    ...
    public void submit(PaintingRenderState paintingRenderState, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, CameraRenderState cameraRenderState) {
        ...
        poseStack.pushPose();
        poseStack.mulPose((Quaternionfc)Axis.YP.rotationDegrees((float)(180 - paintingRenderState.direction.get2DDataValue() * 90)));
    ...
    public void extractRenderState(Painting painting, PaintingRenderState paintingRenderState, float f) {
        ...
        for (int k = 0; k < j; ++k) {
            for (int l = 0; l < i; ++l) {
                ...
                int p = Mth.floor((double)(painting.getY() + (double)n));
                int q = painting.getBlockZ();
                switch (direction) {
                    case NORTH: {
                        o = Mth.floor((double)(painting.getX() + (double)m));
                        break;
                    }
                    case WEST: {
                        q = Mth.floor((double)(painting.getZ() - (double)m));
                        break;
                    }
                    case SOUTH: {
                        o = Mth.floor((double)(painting.getX() - (double)m));
                ...
                paintingRenderState.lightCoordsPerBlock[l + k * i] = LevelRenderer.getLightColor((BlockAndTintGetter)level, new BlockPos(o, p, q));
            }
        }
    }
