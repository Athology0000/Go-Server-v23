...
implements Weighted<Sound> {
    ...
    public WeighedSoundEvents(Identifier identifier, @Nullable String string) {
        if (SharedConstants.DEBUG_SUBTITLES) {
            MutableComponent mutableComponent = Component.literal((String)identifier.getPath());
            if ("FOR THE DEBUG!".equals(string)) {
                mutableComponent = mutableComponent.append((Component)Component.literal((String)" missing").withStyle(ChatFormatting.RED));
            }
            this.subtitle = mutableComponent;
        } else {
            this.subtitle = string == null ? null : Component.translatable((String)string);
        }
    }
