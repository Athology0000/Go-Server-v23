...
implements AutoCloseable {
    ...
    public static CompletableFuture<Optional<TelemetryLogManager>> open(Path path) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                EventLogDirectory eventLogDirectory = EventLogDirectory.open((Path)path, (String)RAW_EXTENSION);
        ...
        if (this.sessionLog == null) {
            this.sessionLog = CompletableFuture.supplyAsync(() -> {
                try {
                    ...
                    return Optional.of(new TelemetryEventLog(fileChannel, (Executor)Util.backgroundExecutor()));
                }
