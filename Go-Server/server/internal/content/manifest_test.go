...
func TestBuildStableManifest_HashesServedCiphertext(t *testing.T) {
	...
	m, err := BuildStableManifest(context.Background(), dir, "https://example.test", "stable", fixedSigningKey(), moduleKey)
	if err != nil {
		t.Fatalf("BuildStableManifest: %v", err)
	}
	if len(m.Modules) != 1 {
		t.Fatalf("want exactly 1 module, got %d", len(m.Modules))
	}
	mod := m.Modules[0]
...
func TestBuildStableManifest_WrongModuleKeyFails(t *testing.T) {
	...
	if _, err := BuildStableManifest(context.Background(), dir, "https://example.test", "stable", fixedSigningKey(), wrongKey); err == nil {
...
func TestBuildStableManifest_SignatureVerifies(t *testing.T) {
	...
	m, err := BuildStableManifest(context.Background(), dir, "https://example.test", "stable", priv, fixedModuleKey())
	if err != nil {
		t.Fatalf("BuildStableManifest: %v", err)
	}
