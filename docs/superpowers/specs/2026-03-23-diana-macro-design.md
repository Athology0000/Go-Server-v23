...
Two new files in `src/main/kotlin/org/cobalt/internal/diana/`:

- **`DianaParticleTracker.kt`** — particle collector. Accepts CRIT particle positions (called from the netty thread), performs direction-averaging from the player position, extends the ray to the ground surface via `Level.getHeight(Heightmap.Types.MOTION_BLOCKING, x, z)`. Thread-safe: `particles` is a `Collections.synchronizedList`. All reads of `particles.size` and snapshot copies use `synchronized(particles) { ... }`.
- **`DianaModule.kt`** — main `Module` subclass. Owns the state machine, subscribes to `TickEvent.Start` and `PacketEvent.Incoming`, drives `NativePathfinder`, renders a box at the predicted burrow via `OverlayRenderEngine`. Holds a `BezierTrackingRotationStrategy` instance stored as a field (not created per-tick).

Two existing files modified:

- **`MovementManager.kt`** — add `forcedAttack` and `forcedUse` fields; clear in `clearForcedMovement()`.
- **`KeyMappingMixin.java`** — replace the existing combined `keyAttack`/`keyUse` block with two separate blocks returning the new flags.

`DianaModule` is registered in `Cobalt.onInitializeClient()`.

## Settings

| Name | Type | Default | Notes |
|---|---|---|---|
| Spade Slot | `SliderSetting` (1.0–9.0, step 1.0) | 1.0 | 1-indexed display; use `.value.toInt() - 1` for `inventory.selected` (0-indexed) |
| Weapon Slot | `SliderSetting` (1.0–9.0, step 1.0) | 2.0 | Same 1-indexed convention |
| Collect Duration | `SliderSetting` (20.0–120.0, step 1.0) | 60.0 | Ticks; use `.value.toInt()` |
| Post-Kill Wait | `SliderSetting` (20.0–200.0, step 1.0) | 80.0 | Ticks; use `.value.toInt()` |
| Min Particles | `SliderSetting` (3.0–30.0, step 1.0) | 8.0 | Minimum count to trust triangulation; use `.value.toInt()` |
...
Always read `NativePathfinder.status` when `tick()` returns null — do not infer state from null alone.

## State Machine

```
IDLE → ACTIVATING_SPADE → COLLECTING_PARTICLES → PATHFINDING → DIGGING → COMBAT → WAITING → IDLE
```
...
- Set `hasForcedMovement = true` and `forcedUse = true`:
  ...
  ```
  **`hasForcedMovement` must be explicitly set via `setForcedMovement(...)` — setting `forcedUse` alone is not enough because `KeyMappingMixin` gates all forced keys on `hasForcedMovement && forcedXxx`.**
...
- When `collectTicksElapsed >= collectDurationTicks`:
  - If `DianaParticleTracker.count() >= minParticles`:
    - `burrowPos = DianaParticleTracker.getBurrowPos(activationPlayerPos, level)` → if non-null: transition to PATHFINDING, `pathfindingTicksElapsed = 0`.
    - If null (degenerate direction): transition to IDLE (retry).
  - If `count() < minParticles`: transition to IDLE (retry).
  - On either retry path: do **not** unlock movement — IDLE re-locks it next tick.
...
- Each tick: increment `pathfindingTicksElapsed`. Call `val cmd = NativePathfinder.tick()`.
  ...
  - If `cmd == null`: check `NativePathfinder.status`:
    - `ARRIVED`, or `player.position().distanceTo(burrowPos) ≤ 2.0`: switch hotbar to spade slot, `digTicksElapsed = 0` → transition to DIGGING.
    ...
    - `PLANNING`: `MovementManager.clearForcedMovement()` — clears stale flags to prevent jump spam. Stay in PATHFINDING.
...
- Scan for live mob: `level.getEntitiesOfClass(LivingEntity::class.java, AABB(burrowPos, burrowPos).inflate(6.0))`. (Use `getEntitiesOfClass` — do **not** use `entitiesForRendering()` which is render-thread-only.)
...
- On mob found: `forcedUse = false`, `targetEntityId = mob.id`, switch hotbar to weapon slot → transition to COMBAT.
...
private val rotationStrategy = BezierTrackingRotationStrategy(
  ...
  // snapThreshold retains its default of 0.25f — matches CombatMacroModule behaviour
)
```
...
- When `waitTicksElapsed >= postKillWaitTicks`: `DianaParticleTracker.reset()` → transition to IDLE.
...
object DianaParticleTracker {
  ...
  // Called from netty thread — Collections.synchronizedList makes add() thread-safe
...
During PATHFINDING and DIGGING, render a box at `burrowPos` each tick. During COLLECTING_PARTICLES, `burrowPos` is null (not yet triangulated) so no box is shown — this is intentional.
...
| `src/main/kotlin/org/cobalt/api/util/player/MovementManager.kt` | Modify |
| `src/main/java/org/cobalt/mixin/client/KeyMappingMixin.java` | Modify |
