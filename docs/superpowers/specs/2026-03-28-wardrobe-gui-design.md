# Wardrobe GUI â€” Design Spec

**Date:** 2026-03-28

## Overview

A custom wardrobe GUI module for Phantom that replaces the vanilla Hypixel SkyBlock wardrobe inventory screen with a rich overlay. Visually and functionally identical to SkyHanni's `CustomWardrobe`, with one key difference: instead of following vanilla wardrobe pages 1â€“3, users can configure exactly which wardrobe sets appear on each of three custom pages via module settings.

Additional feature: automatic set labeling based on armor reforge prefix (Mossyâ†’Farming, Mantidâ†’Pest, Jadedâ†’Mining).

## Scope

**In scope:**
- Intercept and replace the vanilla wardrobe container screen
- Parse all 27 wardrobe sets (3 vanilla pages Ã— 9 slots) from inventory packets
...
- Auto-naming based on reforge prefix (hardcoded: Mossyâ†’Farming, Mantidâ†’Pest, Jadedâ†’Mining)
...
`object WardrobeModule : Module("Wardrobe GUI")` registered in `Phantom.onInitializeClient()`.

**Settings:**

```kotlin
val enabled    by CheckboxSetting("Enabled", "Replace vanilla wardrobe with custom GUI", true)
val page1Slots by TextSetting("Page 1 Slots", "Comma-separated slot numbers (1â€“27)", "1")
val page2Slots by TextSetting("Page 2 Slots", "Comma-separated slot numbers (1â€“27)", "")
```

Page 3 is always the computed remainder â€” any non-empty set whose ID is not claimed by page 1 or 2 appears on page 3. No setting needed.
...
object WardrobeState {
    ...
    var currentVanillaPage: Int?            // which vanilla page (1â€“3) is loaded
    var equippedSlotId: Int?                // slot 1â€“27 that is currently equipped
...
data class WardrobeSet(
    val id: Int,             // 1â€“27
    val vanillaPage: Int,    // 1â€“3 (which vanilla page it lives on)
...
- Vanilla wardrobe layout:
  - Slots 0â€“8: helmets for sets 1â€“9 on the current page
  - Slots 9â€“17: chestplates
  - Slots 18â€“26: leggings
  - Slots 27â€“35: boots
  - Slots 36â€“44: clickable set slots (click these to equip)
  - Slot 48: Back button
  - Slot 49: Close button
- Parse armor from slots 0â€“35 for the current vanilla page's 9 sets
- Detect equipped slot by checking slot 36â€“44 item names for "Â§aEquipped"
- Detect locked by checking for red stained glass pane in slots 36â€“44
...
**FakePlayer creation:** Phantom does not have a `FakePlayer` class. We create one: a `WardrobeFakePlayer` that extends `AbstractClientPlayer` (or wraps a bare `Player` entity constructed against `mc.level`), never added to the level. One instance per `WardrobeSet`, lazily constructed. Armor is applied via `entity.equipment.set(EquipmentSlot, stack)`. Reused across frames; recreated only when the set's armor changes. This is identical in approach to SkyHanni's `FakePlayer.kt`.
...
Mouse coordinates from `GuiRenderEvent` are not directly available â€” use `mc.mouseHandler` for current cursor position.
...
Favorites (`Set<Int>`) and page slot assignments are serialized through Phantom's existing config system (`Config.saveModulesConfig()`). The TextSettings persist automatically. Favorites need a dedicated config field in the module or companion object.

## Data Flow

```
PacketEvent.Incoming (OpenScreen)
  â†’ WardrobeState.isOpen = true, parse page

PacketEvent.Incoming (ContainerSetContent)
  â†’ parse armor for current vanilla page
  â†’ if auto-scanning: advance vanilla page, repeat until all 3 pages scanned

GuiRenderEvent (every frame while wardrobe screen open)
  â†’ WardrobeScreenMixin suppresses vanilla render
  â†’ WardrobeModule.render() draws full custom overlay

Mouse click
  â†’ hit-test against frame's slot hitboxes
  â†’ equip / favorite / navigate
```

## File Layout

```
src/main/kotlin/org/phantom/internal/wardrobe/
...
- `InventoryScreen.renderEntityInInventory()` must be called outside NVG frame (`endFrame()` before, `beginFrame()` after if mixing NVG and entity rendering in the same pass â€” same pattern used by `InventoryHudModule` mixing NVG + `GuiGraphics`).
