...
**Date:** 2026-05-04
**Scope:** CommissionMacroModule (v5 patterns), TunnelMinerModule (multi-ore + vein data), GemstoneMinerModule (route-following etherwarp loop)

---

## 1. CommissionMacroModule — Port rdbt v5 Patterns
...
### 1.2 `updateCommissionsIfChanged()` — Full v5 Rewrite
Replace the current body with:
1. Early return if `commissions == newCommissions`
2. If `ignoreTabUpdatesUntil > now` and `lastCommissionSyncSource == "GUI"` → return (locked after GUI sync)
3. If `ignoreTabUpdatesUntil > now` and `lastCompletedCommissionName != null`:
   - If the new list still has that commission at progress=1 → return (stale data)
   - Otherwise clear `ignoreTabUpdatesUntil`
4. Else if `ignoreTabUpdatesUntil <= now` → clear it
5. Accept new commissions; set `lastCommissionSyncSource = "TAB"`
6. If `awaitingTabUpdate`:
   - Check `stillCompleted` = any commission in the new list has progress≥1 AND resolves to a known task
   - If `!stillCompleted` → `awaitingTabUpdate = false`
   - Else if `lastCompletedCommissionName != null`: if that specific commission is now gone or progress<1 → `awaitingTabUpdate = false`
...
### 1.4 `handleChoosing()` — Guard Order
Replace the current top of `handleChoosing()` with v5 order:
1. `if (shouldWaitForLastCompleted()) return`
2. `if (awaitingTabUpdate) return`
3. `val completed = findCompletedCommission()`
4. If completed found: check double-claim guard — if `completed.name == lastCompletedCommissionName && ignoreTabUpdatesUntil > now` → `awaitingTabUpdate = true; return`
...
### 1.6 `updateCommissionsFromGui()` — New Method
...
### 1.7 `handleClaiming()` — Pigeon Cap + Drill Equip
...
- If `pigeonSlot >= 0`:
  ...
  - If `pigeonAttempts < 3 && !pigeonTimedOut`:
    - If wrong slot → `holdHotbarSlot(pigeonSlot); delay(3)` and return
...
### 1.8 `getAvoidanceEntities()` — Crystal/Star Sentries
...
## 2. TunnelMinerModule — Multi-Ore + Vein Data

### 2.1 Multi-Ore Support
Change `oreOverride: TunnelOreType?` → `oreOverrides: Set<TunnelOreType>`.
...
### 2.2 Vein Data — JSON Resource
- Write `src/main/resources/cobalt/glacite_veins.json` containing the GlaciteData.js coordinates converted to JSON format: `{ "glacite": [[[x,y,z],...], ...], "tungsten": [...], ... }`
...
## 3. GemstoneMinerModule — Route-Following Etherwarp Loop
...
```
IDLE → DECIDING → WARP_TARGETING → WARP_ALIGNING → WARP_USING → WARP_VERIFYING
                                                                        ↓
                                                                    [if MINE point]
                                                                     MINING
                                                                        ↓
...
Routes are loaded from RouteStore via `GemstoneRouteStoreBridge` (see §3.5).

### 3.3 State Machine
**DECIDING:**
- Get `route[routeIndex]` (skip disabled points)
- If already within `WARP_RADIUS` (1.0 blocks) of `standPos` AND point is MINE → go to MINING
- Else → go to WARP_TARGETING

**WARP_TARGETING:**
- Find visible face: `StyleEtherwarpTargeting.findTargetWithRetryProfile(point.lookPos.below())` falling back to `point.lookPos`
- If no face after `maxWarpAttempts` → walk fallback (PathfindingModule) or fail point
- On success → `WARP_ALIGNING`

**WARP_ALIGNING:**
- Apply rotation toward ether target
- After 2 ticks → `WARP_USING`
...
- If within 1.0 blocks of `standPos` → `onArrivedAtPoint()`
- Else if attempts remain → retry WARP_TARGETING
- Else → walk fallback or fail

**MINING:**
- Call `MiningMacroModule.startForAutomation(gemstoneTypeName)`
- Poll `MiningMacroModule.isActive`:
  - If `!isActive && miningStarted` → mining done, advance `routeIndex`, → DECIDING
- Mining timeout → fail point, advance

**FAILED:**
- Advance routeIndex past the bad point, `scanCooldown = 20`, → DECIDING
...
On enable: load route → if empty, send message and disable → else `routeIndex = findClosestRouteIndex()` → `currentState = DECIDING`.

### 3.5 `GemstoneRouteStoreBridge` — Implementation
...
- `routeName: TextSetting` — optional explicit route name
...
| `TunnelMinerModule.kt` | `oreOverride` → `oreOverrides: Set<TunnelOreType>`, update `startForAutomation` + `selectedOres` |
| `TunnelTypes.kt` | `TunnelVeinData.getVeins()` — load from JSON resource |
| `resources/cobalt/glacite_veins.json` | New — GlaciteData.js coordinates as JSON |
