...
data class `CobaltSession-realauth`(
  ...
  companion object {
    val INVALID = `CobaltSession-realauth`("")
    ...
    fun readAndDelete(): `CobaltSession-realauth` {
      ...
      if (fallbackFile.exists()) {
        ...
        if (token.isNotBlank()) {
          return `CobaltSession-realauth`(token, fallbackAlias)
        }
      }
      ...
      return if (fallbackAlias.isNotBlank()) {
        `CobaltSession-realauth`("", fallbackAlias)
      } else {
        INVALID
      }
    }

    private fun readFromProperty(propValue: String, fallbackAlias: String): `CobaltSession-realauth` {
      ...
      if (!possibleFile.exists()) {
        ...
        return if (propValue.isNotBlank()) {
          `CobaltSession-realauth`(propValue, fallbackAlias)
        } else {
          INVALID
        }
      }
      ...
      return try {
        ...
        if (raw == null) {
          if (fallbackAlias.isNotBlank()) `CobaltSession-realauth`("", fallbackAlias) else INVALID
        } else {
          ...
          if (token.isBlank() && alias.isBlank()) {
            ...
            `CobaltSession-realauth`(token, alias)
          }
        }
      } catch (e: Exception) {
