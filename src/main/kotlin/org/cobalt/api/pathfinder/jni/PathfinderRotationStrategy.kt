...
/**
 * PD-controller rotation strategy — direct port of V5 PathRotations.applyHumanizedPhysics().
 *
 *   error * dynamicKp − velocity * dynamicKd → accel → velocity *= 0.92 → angle
 *
 * Includes V5's initialTurnBoostFactor (2× KP/accel on first sharp turn of each path).
...
object PathfinderRotationStrategy : IRotationStrategy {
    ...
    // ── Core rotation (per render frame) ──────────────────────────────────
    ...
    /**
     * Direct port of V5 applyHumanizedPhysics().
     * initialTurnBoostFactor doubles KP, accelLimit, maxVelocity on large initial turns.
     * isStraight increases KD by 1.3× to damp overshoot on straight segments.
     */
    private fun applyPDPhysics() {
        ...
        // V5: decrement initialTurnBoostTicks; zero early when turn is nearly complete
