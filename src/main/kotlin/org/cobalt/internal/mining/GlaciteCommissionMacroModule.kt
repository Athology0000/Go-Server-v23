...
import org.cobalt.internal.mining.tunnels.TunnelOreType
import org.cobalt.internal.pathfinding.PathfindingModule

/**
 * Mirrors V5's GlaciteCommissionMacro.js exactly.
 *
 * Loop: IDLE → CHOOSING → MINING → CLAIMING → WAITING_GUI_CLOSE → CHOOSING
 *
 * No TRAVELING state — TunnelMinerModule handles its own pathfinding to vein edges,
 * exactly like V5's TunnelsMiner.
 *
...
object GlaciteCommissionMacroModule : Module("Glacite Commission Macro") {
    ...
    // V5 SUPPORTED_ORES order (determines priority when multiple commissions active)
    private val SUPPORTED_ORES = listOf("glacite", "umber", "tungsten", "peridot", "aquamarine", "onyx", "citrine")

    // V5 EMISSARY_LOCATION = [2, 121, 237]
    ...
    // Claiming state (mirrors V5 pigeonAttempts / firstPigeonAttemptAt)
    ...
    fun onTick(@Suppress("UNUSED_PARAMETER") event: TickEvent.Start) {
        ...
        // V5 reads tab on every step (1-tick frequency)
    ...
    private fun handleMining() {
        ...
        // V5: if (!Pathfinder.isPathing() && !MiningBot.enabled && now - lastTunnelRestartAt >= 5000)
    ...
    private fun handleClaiming() {
        ...
        // V5: pigeonAttempts < 3 && !pigeonTimedOut
    ...
    private fun claimCompletedCommissions(screen: AbstractContainerScreen<*>) {
        ...
        // No completed slots left — done claiming; sync commission list from GUI (V5 updateCommissionsFromGui)
