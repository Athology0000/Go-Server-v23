...
import org.phantom.mixin.client.ChatComponentAccessor
...
object ChatUtils {
  ...
  fun refreshAnimatedChat(chat: ChatComponent) {
    ...
    val messages = accessor.getPhantomAllMessages()
    var changed = false

    for (index in messages.indices) {
      val message = messages[index]
      val replacement = animatedPhantomLine(message.content().string) ?: continue
      messages[index] = GuiMessage(message.addedTime(), replacement, message.signature(), message.tag())
      changed = true
    }

    if (changed) {
      accessor.callPhantomRefreshTrimmedMessages()
    }
  }
  ...
  fun animatedChatLineContent(original: FormattedCharSequence): FormattedCharSequence? {
    ...
    val replacement = animatedPhantomLine(raw) ?: return null
    return replacement.visualOrderText
  }

  private fun animatedPhantomLine(raw: String): MutableComponent? {
    val clean = stripFormatting(raw)
    return when {
      clean.startsWith("[Phantom Debug] ") -> Component.empty()
        .append(debugPrefix())
        .append(glowingGradient(clean.removePrefix("[Phantom Debug] "), 0xEFFFFC, 0x78FFD2))

      clean.startsWith("[Phantom] ") -> Component.empty()
        .append(prefix())
        .append(glowingGradient(clean.removePrefix("[Phantom] "), 0xF7FCFF, 0x73DFFF))
  ...
  private fun prefix(): MutableComponent = Component.literal("[")
    ...
    .append(glowingGradient("Phantom", 0x3A9ED8, 0xB9F9FF))
    .append(Component.literal("] ").withStyle(ChatFormatting.DARK_GRAY))

  private fun debugPrefix(): MutableComponent = Component.literal("[")
    .withStyle(ChatFormatting.DARK_GRAY)
    .append(glowingGradient("Phantom Debug", 0x2ECF8A, 0xB9FFF1))
    .append(Component.literal("] ").withStyle(ChatFormatting.DARK_GRAY))
