...
import org.phantom.internal.combat.slayer.BlazeSlayerSettings
import org.phantom.internal.combat.slayer.EndermanSlayerPhase
import org.phantom.internal.combat.slayer.EndermanSlayerSettings
import org.phantom.internal.combat.slayer.SlayerLocationSettings
import org.phantom.internal.combat.slayer.SlayerQuestSignals
import org.phantom.internal.combat.slayer.SlayerDefinitions
import org.phantom.internal.combat.slayer.SpiderSlayerPhase
import org.phantom.internal.combat.slayer.SpiderSlayerSettings
import org.phantom.internal.combat.slayer.VampireSlayerSettings
import org.phantom.internal.combat.slayer.WolfSlayerSettings
import org.phantom.internal.combat.slayer.ZombieSlayerSettings
...
object CombatMacroModule : Module("Combat Macro") {
  ...
  // Cached per-tick: entity UUID â†’ highlight type, used by renderSlayerHighlights to avoid O(NÂ²) per frame.
  ...
  private fun resolveSlayerTarget(player: Player, level: ClientLevel): LivingEntity? {
    ...
    for (entity in level.entitiesForRendering()) {
      ...
      // Compute names once â€” avoids 3 separate findAttachedArmorStandNames scans per entity.
  ...
  private fun readScoreboardSidebarLines(level: ClientLevel?): List<String> {
    ...
    return org.phantom.api.util.ScoreboardUtils.sidebarLines()
  ...
  private fun rangedAimRotation(
    ...
    target: LivingEntity,
    activeCombatMode: Int = combatMode.value
  ): org.phantom.api.util.helper.Rotation {
    ...
    return org.phantom.api.util.helper.Rotation(base.yaw, base.pitch - correction)
  }
  ...
  private fun resolveSlayerPhaseTarget(
    ...
    for (entity in level.entitiesForRendering()) {
      ...
      if (living is ArmorStand) {
        // "shoot me" targets are armor stands â€” allow them only during spider hatchlings phase
  ...
  private fun renderSlayerHighlights(context: org.phantom.api.event.impl.render.WorldRenderContext, level: ClientLevel) {
    val cache = slayerHighlightCache
    if (cache.isEmpty()) return
    val espMode = slayerEspTargets.value
    // Use the tick-computed cache to avoid re-running the O(NÂ²) armor-stand scan every frame.
    for (entity in level.entitiesForRendering()) {
      val living = entity as? LivingEntity ?: continue
  ...
  private val SANITIZE_COLOR_CODES   = Regex("Â§.")
