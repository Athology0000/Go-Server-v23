...
import org.phantom.api.util.getHeadTextureId
import org.phantom.api.util.render.Render3D
import org.phantom.internal.dungeons.map.DoorKind
import org.phantom.internal.dungeons.map.DungeonDoor
...
object BloodCampHelperModule : Module("Blood Camp Helper") {
  ...
  private fun nextDoorBox(door: DungeonDoor): AABB {
    ...
    return if ((door.component.x and 1) == 1) {
      // Door runs along X axis â€” 1 block thick in X, 3 blocks wide in Z
      AABB(
        center.x + 0.5 - 0.5,
        minY,
        center.z + 0.5 - 1.5,
      ...
      // Door runs along Z axis â€” 3 blocks wide in X, 1 block thick in Z
      AABB(
        center.x + 0.5 - 1.5,
        minY,
        center.z + 0.5 - 0.5,
