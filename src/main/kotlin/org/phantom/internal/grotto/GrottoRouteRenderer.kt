...
object GrottoRouteRenderer {
  ...
  fun onRender(event: WorldRenderEvent.Last) {
    if (!FairyGrottoModule.renderRoutes.value) {
  ...
  fun tick(client: Minecraft) {
    ...
    if (FairyGrottoModule.routeObstructionHighlights.value) {
  ...
  fun renderLabels(context: org.phantom.api.event.impl.render.WorldRenderContext) {
  ...
  private fun renderGradientLines(context: org.phantom.api.event.impl.render.WorldRenderContext) {
  ...
  private fun renderPointOutlines(context: org.phantom.api.event.impl.render.WorldRenderContext) {
    if (routePoints.isEmpty()) return
    val segments = max(1, routePoints.size - 1)
