...
object GrottoScanner {
  ...
  private fun syncSetting() {
    val desired = FairyGrottoModule.scannerEnabled.value
    if (desired != enabled) {
  ...
  private fun setEnabled(value: Boolean, announce: Boolean) {
    ...
    enabled = value
    FairyGrottoModule.scannerEnabled.value = value
  ...
  fun initBlacklistFile() {
    if (blacklistFile == null) {
      blacklistFile = File(mc.gameDirectory, "config/phantom/grotto_blacklist.json")
    }
    if (!blacklistLoaded) {
      loadUserBlacklist()
  ...
  fun onRender(event: WorldRenderEvent.Last) {
    ...
    if (FairyGrottoModule.scannerRenderBoxes.value) {
    ...
    if (FairyGrottoModule.scannerRenderTracers.value) {
