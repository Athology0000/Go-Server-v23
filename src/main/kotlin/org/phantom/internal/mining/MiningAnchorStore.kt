 ...
 *   - Y < 189  â†’ Dwarven Mines  â†’  dwarven_data.json
 *   - Y â‰¥ 189  â†’ Glacite Tunnels â†’ glacite_data.json
 *
...
internal object MiningAnchorStore {
    ...
    private fun fileFor(area: MiningArea): File =
        File(Minecraft.getInstance().gameDirectory, "config/phantom/${area.fileName}")

    private fun configDir(): File =
        File(Minecraft.getInstance().gameDirectory, "config/phantom")
    ...
    private fun saveToDisk(area: MiningArea, anchors: Set<Long>) {
        ...
        try {
            ...
            // Silently ignore â€” anchors are still kept in-memory for this session.
        }
    }
}
