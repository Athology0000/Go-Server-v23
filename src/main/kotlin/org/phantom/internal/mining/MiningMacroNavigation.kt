...
internal fun MiningMacroModule.moveToward(
  ...
  if (approachTarget != target) {
    ...
    // Pathfinder is still searching â€” defer the timeout clock so planning latency
    // doesn't eat into the execution budget.
    approachStartTick = level.gameTime
    approachStartDistance = currentDistance
  } else if (approachStartDistance - currentDistance >= APPROACH_MIN_PROGRESS_BLOCKS) {
    // Fresh progress â€” slide the baseline forward so a later stall still trips the timeout.
    approachStartTick = level.gameTime
    approachStartDistance = currentDistance
...
internal fun MiningMacroModule.focusApproachTarget(player: Player, target: BlockPos) {
  ...
  // fields below are no longer wired to a renderer â€” without this request the
 ...
 * air. The macro should never aim at a buried face â€” pathing/breaking can't
...
internal fun MiningMacroModule.buildAirFacingAimPoints(
  ...
  // pick a face the camera is nearly grazing â€” those produce sub-pixel aim
  ...
  val usable = faces.mapNotNull { (point, dir, normal) ->
    ...
    // Require at least ~17Â° away from grazing (cos > 0.30). Tighter than the
    // previous 0.05 epsilon â€” a face that's only 3Â° off the silhouette is
...
internal fun MiningMacroModule.findVisibleAimPoint(
  ...
  // ray geometrically lines up â€” the renderer/server will reject it.
  ...
  // mine anyway â€” but the original code took inset samples that could still
 ...
 * occluder (bedrock, etc. â€” see [MiningBlockRegistry.PERMANENT_LOS_OCCLUDERS])
...
internal fun MiningMacroModule.canSeeAimPoint(
  ...
  // Reject if the ray landed on a face whose neighbor is NOT air â€” that face
