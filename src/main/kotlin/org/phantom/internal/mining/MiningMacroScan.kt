 ...
 *  1. A route is loaded for [org.phantom.internal.routes.RouteType.ORE_MINER]:
...
internal fun MiningMacroModule.validateManualStartPreconditions(
  ...
  if (pickedRouteName.isNotEmpty()) {
    val route = org.phantom.internal.routes.RouteStore.loadAll().firstOrNull {
      it.name == pickedRouteName && it.type == org.phantom.internal.routes.RouteType.ORE_MINER
    }
    if (route != null) {
      val minePoints = (route.loopOrArea + route.travelRoute).filter {
        it.type == org.phantom.internal.routes.RoutePointType.MINE ||
          it.type == org.phantom.internal.routes.RoutePointType.VEIN
      }
      if (minePoints.isNotEmpty()) {
        ...
        org.phantom.internal.routes.RouteStore.setLoaded(route)
...
/** Search the 3Ã—3Ã—3 cube around [anchor] for a mineable seed. */
