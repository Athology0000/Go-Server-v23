...
import org.phantom.api.rotation.strategy.HeadRotationStrategy
...
import org.phantom.internal.routes.RoutePointType as NewRoutePointType
...
object RoutesModule : Module("Routes") {
  ...
  private val routesDirectory = File(mc.gameDirectory, "config/phantom/routes")
  private val legacyRoutesFile = File(mc.gameDirectory, "config/phantom/routes.json")
  ...
  private val rdbtRouteWalker = CheckboxSetting(
    "RDBT Route Walker",
    "Use the RDBT/Phantom-style direct walker for walk/etherwarp routes.",
    true
  )
  ...
  private val openPickerAction = ActionSetting(
    ...
    "Open"
  ) { /* point picker popup removed â€” use the Routes screen edit mode */ }
    ...
    if (action == RouteAction.WALK) {
      if (nativeActive()) {
        ...
        val isTeleportAction = cmd != null &&
          (cmd.activeAction == org.phantom.api.pathfinder.jni.ActionType.AOTV ||
           cmd.activeAction == org.phantom.api.pathfinder.jni.ActionType.ETHERWARP)
  ...
  /**
   * Bridge: read a route saved by the new [RouteStore] (config/phantom/routes2/) and
   ...
   * - Point types: WALK â†’ NORMAL, WARP â†’ WARP, MINE/VEIN â†’ MINE, LANTERN/KILL â†’ skipped.
   */
  ...
  private fun listSavedRouteNames(): List<String> {
    ...
    // Bridge: routes created by the new picker live in config/phantom/routes2/ and are
  ...
  private fun moveToward(
    ...
    // Already adjacent to the approach block â€” stop re-pathing to prevent oscillation.
