...
object VeinScannerModule : Module("Vein Scanner") {
  ...
  private val exportFile = TextSetting(
    "Export File",
    "JSON file written under config/phantom/vein-scans.",
  ...
  private fun captureCurrentVein(forceRescan: Boolean = false) {
    ...
    ChatUtils.sendMessage(
      if (added) {
        "Vein Scanner: exported ${vein.ore.displayName} vein (${vein.blocks.size} blocks) to config/phantom/vein-scans/${exportFileName()}."
      } else {
...
object VeinScanStore {
  ...
  private fun dir(): File = File(Minecraft.getInstance().gameDirectory, "config/phantom/vein-scans")
