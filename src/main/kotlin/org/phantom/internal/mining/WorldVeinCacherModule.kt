 ...
 * into per-(material, area) JSON files in config/phantom/:
 *
 ...
 *   ...
 *
 * Files are append-only in practice â€” re-walking the same ground costs nothing
...
object WorldVeinCacherModule : Module("World Vein Cacher") {
    ...
    private val info = InfoSetting(
        "World Vein Cacher",
        "Toggle on and walk around â€” every vein you pass gets saved.",
        InfoType.INFO,
    )
    ...
    private val clearAllButton = ActionSetting(
        "Delete All Caches",
        "Erase every *_dwarven_data.json / *_glacite_data.json in config/phantom.",
        "Clear",
    ) {
        deleteAllCacheFiles()
    }
    ...
    private fun configDir(): File? {
        val dir = File(Minecraft.getInstance().gameDirectory, "config/phantom")
