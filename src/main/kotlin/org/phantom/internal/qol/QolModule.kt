...
import org.phantom.api.module.setting.impl.CommandHotkeySetting
import org.phantom.api.module.setting.impl.CommandHotkeyValue
...
object QolModule : Module("QoL") {
  ...
  fun onTick(@Suppress("UNUSED_PARAMETER") event: TickEvent.Start) {
    commandHotkeys.forEachIndexed { index, entry ->
      if (!enabled.value && !isDefaultPhantomMenuHotkey(index, entry.value)) return@forEachIndexed
  ...
  private fun isDefaultPhantomMenuHotkey(index: Int, value: CommandHotkeyValue): Boolean {
    return index == 0 &&
  ...
  private fun loadSavedHotkeyCount(): Int {
    ...
    val file = File(dir, "config/phantom/addons.json")
    ...
    return runCatching {
      ...
      json.forEach { addonEl ->
        ...
        if (addonObj.get("addon")?.asString != "phantom") return@forEach
