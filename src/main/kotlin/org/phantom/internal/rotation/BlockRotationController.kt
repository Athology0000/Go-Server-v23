...
class BlockRotationController {
    ...
    // Real-time progress (seconds) â€” drives smooth per-frame interpolation.
    ...
    /** Precision point received before we reached the block edge â€” applied on arrival. */
    ...
    fun rotate(newRequest: BlockRotationRequest) {
        ...
        // mining macro for the next ~20 frames â€” the camera stayed locked on
        ...
        // tracking â€” keep its existing value so velocity stays continuous.
     ...
     * the live lookahead rotation. The eased curve is bypassed â€” the exp
    ...
    fun setPrecisionPoint(point: Vec3?) {
        ...
        if (!arrivedAtBlockEdge && request != null) {
            // Still rotating toward the random face spot â€” stage for handoff.
     ...
     * from start â†’ target rotation using the request's easing curve.
     *
    ...
    private fun update(dt: Double) {
        ...
        // Settle test â€” finished only when the smoother has actually caught
        ...
        // Phase 1 â†’ 2 transition. Once we've reached the random visible-face
