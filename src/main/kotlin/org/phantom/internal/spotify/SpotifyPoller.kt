...
object SpotifyPoller {
    ...
    private val scriptFile: File by lazy {
        File(System.getProperty("java.io.tmpdir"), "phantom_spotify_smtc.ps1").also { f ->
            f.writeText(PS_SCRIPT)
        }
    }

    private val meterScriptFile: File by lazy {
        File(System.getProperty("java.io.tmpdir"), "phantom_spotify_meter.ps1").also { f ->
...
try {
    ...
    try {
        if (${'$'}null -ne ${'$'}p.Thumbnail) {
            ...
            ${'$'}artOut = [System.IO.Path]::Combine([System.IO.Path]::GetTempPath(), "phantom_smtc_art.jpg")
...
public class PhantomKbd {
    [DllImport("user32.dll")] public static extern void keybd_event(byte bVk, byte bScan, uint dwFlags, uint dwExtraInfo);
}
'@
PhantomKbd::keybd_event($vkCode, 0, 0, 0)
PhantomKbd::keybd_event($vkCode, 0, 2, 0)
                """.trimIndent()
                ProcessBuilder(
    ...
    private fun fetchArt(artist: String, title: String) {
        try {
            ...
            artFileCounter++
            File(System.getProperty("java.io.tmpdir"), "phantom_art_${artFileCounter - 1}.jpg").delete()
            val f = File(System.getProperty("java.io.tmpdir"), "phantom_art_$artFileCounter.jpg")
            f.writeBytes(bytes)
