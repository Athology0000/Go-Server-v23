...
object CustomScoreboardModule : Module("Custom Scoreboard") {
  ...
  private fun footerRows(): List<ScoreboardRow> {
    val footer = if (footerUseCustomText.value) customFooterText() else "phantom client"
    val lines = footer.replace("\\n", "\n").split('\n')
    return lines.map { line ->
      val component = if (line.equals("phantom client", ignoreCase = true)) {
  ...
  private fun customFooterText(): String {
    ...
    return if (alpha) {
      ...
      footerText.value.ifBlank { "phantom client" }
    }
  }
  ...
  private fun collectTabLines(): List<String> {
    ...
    lines += org.phantom.api.util.TabListUtils.rawDisplayNames().flatMap(::splitTabText)
