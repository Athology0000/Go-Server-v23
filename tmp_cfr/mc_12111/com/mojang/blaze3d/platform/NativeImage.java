...
implements AutoCloseable {
    ...
    public static NativeImage read(@Nullable Format format, ByteBuffer byteBuffer) throws IOException {
        ...
        PngInfo.validateHeader((ByteBuffer)byteBuffer);
    ...
    public int getPixel(int i, int j) {
        return ARGB.fromABGR((int)this.getPixelABGR(i, j));
    }
    ...
    public void setPixel(int i, int j, int k) {
        this.setPixelABGR(i, j, ARGB.toABGR((int)k));
    }
    ...
    public NativeImage mappedCopy(IntUnaryOperator intUnaryOperator) {
        ...
        for (int j = 0; j < i; ++j) {
            int k = ARGB.fromABGR((int)intBuffer.get(j));
            int l = intUnaryOperator.applyAsInt(k);
            intBuffer2.put(j, ARGB.toABGR((int)l));
        }
        return nativeImage;
    }
