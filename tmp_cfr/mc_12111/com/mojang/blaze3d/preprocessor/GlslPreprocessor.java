    ...
    private List<String> processImports(String string, Context context, String string2) {
        ...
        while (matcher.find()) {
            ...
            if (!Strings.isNullOrEmpty((String)string7)) {
                ...
                ++context.sourceId;
                k = context.sourceId;
                List<String> list2 = this.processImports((String)string7, context, bl ? FileUtil.getFullResourcePath((String)string6) : "");
                list2.set(0, String.format(Locale.ROOT, "#line %d %d\n%s", 0, k, this.processVersions(list2.get(0), context)));
                if (!StringUtil.isBlank((String)string5)) {
                    list.add(string5);
                }
                list.addAll(list2);
            } else {
            ...
            k = StringUtil.lineCount((String)string.substring(0, matcher.end(1)));
            string3 = String.format(Locale.ROOT, "#line %d %d", k, i);
            j = matcher.end(1);
        }
        string4 = string.substring(j);
        if (!StringUtil.isBlank((String)string4)) {
