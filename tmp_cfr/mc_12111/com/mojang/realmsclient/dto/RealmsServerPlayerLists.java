...
public record RealmsServerPlayerLists(Map<Long, List<ResolvableProfile>> servers) {
    ...
    public static RealmsServerPlayerLists parse(String string) {
        ...
        try {
            JsonObject jsonObject = GsonHelper.parse((String)string);
            if (GsonHelper.isArrayNode((JsonObject)jsonObject, (String)"lists")) {
                ...
                for (JsonElement jsonElement : jsonArray) {
                    ...
                    List<Object> list = string2 != null ? ((jsonElement2 = LenientJsonParser.parse((String)string2)).isJsonArray() ? RealmsServerPlayerLists.parsePlayers(jsonElement2.getAsJsonArray()) : Lists.newArrayList()) : Lists.newArrayList();
    ...
    private static List<ResolvableProfile> parsePlayers(JsonArray jsonArray) {
        ...
        for (JsonElement jsonElement : jsonArray) {
            ...
            list.add(ResolvableProfile.createUnresolved((UUID)uUID));
        }
        return list;
    }
