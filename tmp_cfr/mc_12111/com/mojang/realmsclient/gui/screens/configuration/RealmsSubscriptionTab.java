...
implements RealmsConfigurationTab {
    ...
    RealmsSubscriptionTab(RealmsConfigureWorldScreen realmsConfigureWorldScreen, Minecraft minecraft, RealmsServer realmsServer) {
        ...
        rowHelper.addChild(SpacerElement.height(2));
        rowHelper.addChild(Button.builder((Component)Component.translatable((String)"mco.configure.world.subscription.extend"), button -> ConfirmLinkScreen.confirmLinkNow((Screen)realmsConfigureWorldScreen, CommonLinks.extendRealms((String)realmsServer.remoteSubscriptionId, (UUID)minecraft.getUser().getProfileId()))).bounds(0, 0, 200, 20).build());
        rowHelper.addChild(SpacerElement.height(2));
        this.deleteButton = rowHelper.addChild(Button.builder((Component)Component.translatable((String)"mco.configure.world.delete.button"), button -> minecraft.setScreen(RealmsPopups.warningPopupScreen(realmsConfigureWorldScreen, (Component)Component.translatable((String)"mco.configure.world.delete.question.line1"), popupScreen -> this.deleteRealm()))).bounds(0, 0, 200, 20).build());
        rowHelper.addChild(SpacerElement.height(2));
        this.subscriptionInfo = rowHelper.addChild(FocusableTextWidget.builder((Component)Component.empty(), font).maxWidth(200).build(), LayoutSettings.defaults().alignHorizontallyCenter());
        this.subscriptionInfo.setCentered(false);
        this.updateData(realmsServer);
    }

    private void deleteRealm() {
        RealmsUtil.runAsync(realmsClient -> realmsClient.deleteRealm(this.serverData.id), RealmsUtil.openScreenAndLogOnFailure(this.configurationScreen::createErrorScreen, "Couldn't delete world")).thenRunAsync(() -> this.minecraft.setScreen(this.configurationScreen.getLastScreen()), (Executor)((Object)this.minecraft));
    ...
    private static Component localPresentation(Instant instant) {
        String string = ZonedDateTime.ofInstant(instant, ZoneId.systemDefault()).format(Util.localizedDateFormatter((FormatStyle)FormatStyle.MEDIUM));
        return Component.literal((String)string).withStyle(ChatFormatting.GRAY);
    }
    ...
    private Component daysLeftPresentation(int i) {
        ...
        boolean bl3 = bl2 = k > 0;
        if (bl && bl2) {
            return Component.translatable((String)"mco.configure.world.subscription.remaining.months.days", (Object[])new Object[]{j, k}).withStyle(ChatFormatting.GRAY);
        }
        if (bl) {
            return Component.translatable((String)"mco.configure.world.subscription.remaining.months", (Object[])new Object[]{j}).withStyle(ChatFormatting.GRAY);
        }
        if (bl2) {
            return Component.translatable((String)"mco.configure.world.subscription.remaining.days", (Object[])new Object[]{k}).withStyle(ChatFormatting.GRAY);
        }
    ...
    public void updateData(RealmsServer realmsServer) {
        ...
        if (bl) {
            this.subscriptionInfo.setMessage((Component)Component.translatable((String)"mco.snapshot.subscription.info", (Object[])new Object[]{realmsServer.parentWorldName}));
        } else {
