...
public class ColorLerper {
    ...
    public static int getLerpedColor(Type type, float f) {
        int i = Mth.floor((float)f);
        int j = i / type.colorDuration;
        int k = type.colors.length;
        int l = j % k;
        int m = (j + 1) % k;
        float g = ((float)(i % type.colorDuration) + Mth.frac((float)f)) / (float)type.colorDuration;
        int n = type.getColor(type.colors[l]);
        int o = type.getColor(type.colors[m]);
        return ARGB.srgbLerp((float)g, (int)n, (int)o);
    }
    ...
    static int getModifiedColor(DyeColor dyeColor, float f) {
        ...
        return ARGB.color((int)255, (int)Mth.floor((float)((float)ARGB.red((int)i) * f)), (int)Mth.floor((float)((float)ARGB.green((int)i) * f)), (int)Mth.floor((float)((float)ARGB.blue((int)i) * f)));
    }
