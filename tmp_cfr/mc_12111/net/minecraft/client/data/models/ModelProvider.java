...
implements DataProvider {
    ...
    implements ItemModelOutput {
        ...
        public void finalizeAndValidate() {
            BuiltInRegistries.ITEM.forEach(item -> {
                ...
                if (item instanceof BlockItem && !this.itemInfos.containsKey(blockItem = (BlockItem)item)) {
                    ...
                    this.accept((Item)blockItem, ItemModelUtils.plainModel(identifier));
                }
            });
            ...
            List<Identifier> list = BuiltInRegistries.ITEM.listElements().filter(reference -> !this.itemInfos.containsKey(reference.value())).map(reference -> reference.key().identifier()).toList();
            if (!list.isEmpty()) {
        ...
        public CompletableFuture<?> save(CachedOutput cachedOutput, PackOutput.PathProvider pathProvider) {
            return DataProvider.saveAll((CachedOutput)cachedOutput, ClientItem.CODEC, item -> pathProvider.json(item.builtInRegistryHolder().key().identifier()), this.itemInfos);
        }
    }
    ...
    implements Consumer<BlockModelDefinitionGenerator> {
        ...
        public void validate() {
            ...
            List<Identifier> list = stream.filter(reference -> !this.generators.containsKey(reference.value())).map(reference -> reference.key().identifier()).toList();
            if (!list.isEmpty()) {
        ...
        public CompletableFuture<?> save(CachedOutput cachedOutput, PackOutput.PathProvider pathProvider) {
            ...
            return DataProvider.saveAll((CachedOutput)cachedOutput, BlockModelDefinition.CODEC, function, (Map)map);
        }
    ...
    implements BiConsumer<Identifier, ModelInstance> {
        ...
        public CompletableFuture<?> save(CachedOutput cachedOutput, PackOutput.PathProvider pathProvider) {
            return DataProvider.saveAll((CachedOutput)cachedOutput, Supplier::get, arg_0 -> ((PackOutput.PathProvider)pathProvider).json(arg_0), this.models);
        }
