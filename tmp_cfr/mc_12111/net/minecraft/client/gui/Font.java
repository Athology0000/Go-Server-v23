...
public class Font {
    ...
    public void drawInBatch8xOutline(FormattedCharSequence formattedCharSequence, float f, float g, int i, int j, Matrix4f matrix4f, MultiBufferSource multiBufferSource, int k) {
        ...
        formattedCharSequence.accept((FormattedCharSink)preparedTextBuilder2);
    ...
    BakedGlyph getGlyph(int i, Style style) {
        ...
        if (style.isObfuscated() && i != 32) {
            int j = Mth.ceil((float)bakedGlyph.info().getAdvance(false));
    ...
    public PreparedText prepareText(String string, float f, float g, int i, boolean bl, int j) {
        ...
        StringDecomposer.iterateFormatted((String)string, (Style)Style.EMPTY, (FormattedCharSink)preparedTextBuilder);
    ...
    public PreparedText prepareText(FormattedCharSequence formattedCharSequence, float f, float g, int i, boolean bl, boolean bl2, int j) {
        ...
        formattedCharSequence.accept((FormattedCharSink)preparedTextBuilder);
        return preparedTextBuilder;
    }

    public int width(String string) {
        return Mth.ceil((float)this.splitter.stringWidth(string));
    }

    public int width(FormattedText formattedText) {
        return Mth.ceil((float)this.splitter.stringWidth(formattedText));
    }

    public int width(FormattedCharSequence formattedCharSequence) {
        return Mth.ceil((float)this.splitter.stringWidth(formattedCharSequence));
    }
    ...
    PreparedText {
        ...
        private int getTextColor(@Nullable TextColor textColor) {
            if (textColor != null) {
                int i = ARGB.alpha((int)this.color);
        ...
        private int getShadowColor(Style style, int i) {
            ...
            if (integer != null) {
                float f = ARGB.alphaFloat((int)i);
                float g = ARGB.alphaFloat((int)integer);
                if (f != 1.0f) {
                    return ARGB.color((int)ARGB.as8BitChannel((float)(f * g)), (int)integer);
                }
                return integer;
            }
            if (this.drawShadow) {
                return ARGB.scaleRGB((int)i, (float)0.25f);
            }
            return 0;
        }
        ...
        public @Nullable ScreenRectangle bounds() {
            ...
            int i = Mth.floor((float)this.left);
            int j = Mth.floor((float)this.top);
            int k = Mth.ceil((float)this.right);
            int l = Mth.ceil((float)this.bottom);
