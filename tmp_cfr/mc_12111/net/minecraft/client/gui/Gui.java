...
public class Gui {
    ...
    private void renderCameraOverlays(GuiGraphics guiGraphics, DeltaTracker deltaTracker) {
        ...
        this.scopeScale = Mth.lerp((float)(0.5f * f), (float)this.scopeScale, (float)1.125f);
        if (this.minecraft.options.getCameraType().isFirstPerson()) {
            if (localPlayer.isScoping()) {
                ...
                for (EquipmentSlot equipmentSlot : EquipmentSlot.values()) {
                    ...
                    this.renderTextureOverlay(guiGraphics, ((Identifier)equippable.cameraOverlay().get()).withPath(string -> "textures/" + string + ".png"), 1.0f);
                }
            }
        }
        ...
        float h = Mth.lerp((float)g, (float)localPlayer.oPortalEffectIntensity, (float)localPlayer.portalEffectIntensity);
    ...
    private void renderOverlayMessage(GuiGraphics guiGraphics, DeltaTracker deltaTracker) {
        ...
        if (i > 0) {
            ...
            int j = this.animateOverlayMessageColor ? Mth.hsvToArgb((float)(f / 50.0f), (float)0.7f, (float)0.6f, (int)i) : ARGB.white((int)i);
    ...
    private void renderTitle(GuiGraphics guiGraphics, DeltaTracker deltaTracker) {
        ...
        if ((i = Mth.clamp((int)i, (int)0, (int)255)) > 0) {
            guiGraphics.nextStratum();
            guiGraphics.pose().pushMatrix();
    ...
    private void renderChat(GuiGraphics guiGraphics, DeltaTracker deltaTracker) {
        if (!this.chat.isChatFocused()) {
            ...
            int i = Mth.floor((double)this.minecraft.mouseHandler.getScaledXPos(window));
            int j = Mth.floor((double)this.minecraft.mouseHandler.getScaledYPos(window));
            guiGraphics.nextStratum();
        ...
        if (!this.minecraft.debugEntries.isCurrentlyEnabled(DebugScreenEntries.THREE_DIMENSIONAL_CROSSHAIR)) {
            ...
            if (this.minecraft.options.attackIndicator().get() == AttackIndicatorStatus.CROSSHAIR) {
                ...
                if (this.minecraft.crosshairPickEntity != null && this.minecraft.crosshairPickEntity instanceof LivingEntity && f >= 1.0f) {
                    ...
                    bl &= attackRange == null || attackRange.isInRange((LivingEntity)this.minecraft.player, this.minecraft.hitResult.getLocation());
                }
    ...
    private boolean canRenderCrosshairForSpectator(@Nullable HitResult hitResult) {
        ...
        if (hitResult.getType() == HitResult.Type.BLOCK) {
            ...
            return level.getBlockState(blockPos).getMenuProvider((Level)level, blockPos) != null;
        }
        return false;
    }
        ...
        for (MobEffectInstance mobEffectInstance : Ordering.natural().reverse().sortedCopy((Iterable)collection)) {
            ...
            if (mobEffectInstance.isAmbient()) {
                ...
                if (mobEffectInstance.endsWithin(200)) {
                    ...
                    int n = 10 - m / 20;
                    f = Mth.clamp((float)((float)m / 10.0f / 5.0f * 0.5f), (float)0.0f, (float)0.5f) + Mth.cos((double)((float)m * (float)Math.PI / 5.0f)) * Mth.clamp((float)((float)n / 10.0f * 0.25f), (float)0.0f, (float)0.25f);
                    f = Mth.clamp((float)f, (float)0.0f, (float)1.0f);
                }
            }
            guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, Gui.getMobEffectSprite((Holder<MobEffect>)holder), k + 3, l + 3, 18, 18, ARGB.white((float)f));
        }
    }
    ...
    private void renderSelectedItemName(GuiGraphics guiGraphics) {
        ...
        if (this.toolHighlightTimer > 0 && !this.lastToolHighlight.isEmpty()) {
            ...
            if (l > 0) {
                guiGraphics.drawStringWithBackdrop(this.getFont(), (Component)mutableComponent, j, k, i, ARGB.white((int)l));
            }
        }
        Profiler.get().pop();
    }
    ...
    private void renderDemoOverlay(GuiGraphics guiGraphics, DeltaTracker deltaTracker) {
        ...
        guiGraphics.nextStratum();
        Object component = this.minecraft.level.getGameTime() >= 120500L ? DEMO_EXPIRED_TEXT : Component.translatable((String)"demo.remainingTime", (Object[])new Object[]{StringUtil.formatTickDuration((int)((int)(120500L - this.minecraft.level.getGameTime())), (float)this.minecraft.level.tickRateManager().tickrate())});
    ...
    private void displayScoreboardSidebar(GuiGraphics guiGraphics, Objective objective) {
        ...
        DisplayEntry[] lvs = (DisplayEntry[])scoreboard.listPlayerScores(objective).stream().filter(playerScoreEntry -> !playerScoreEntry.isHidden()).sorted(SCORE_DISPLAY_ORDER).limit(15L).map(playerScoreEntry -> {
            ...
            MutableComponent component2 = PlayerTeam.formatNameForTeam((Team)playerTeam, (Component)component);
            MutableComponent component3 = playerScoreEntry.formatValue(numberFormat);
            int i = this.getFont().width((FormattedText)component3);
            return new DisplayEntry((Component)component2, (Component)component3, i);
        }).toArray(i -> new DisplayEntry[i]);
        Component component = objective.getDisplayName();
        int j = i2 = this.getFont().width((FormattedText)component);
    ...
    private void renderPlayerHealth(GuiGraphics guiGraphics) {
        ...
        int i = Mth.ceil((float)player.getHealth());
        ...
        int o = Mth.ceil((float)player.getAbsorptionAmount());
        int p = Mth.ceil((float)((f + (float)o) / 2.0f / 10.0f));
        int q = Math.max(10 - (p - 2), 3);
        int r = n - 10;
        int s = -1;
        if (player.hasEffect(MobEffects.REGENERATION)) {
            s = this.tickCount % Mth.ceil((float)(f + 5.0f));
        }
    ...
    private void renderHearts(GuiGraphics guiGraphics, Player player, int i, int j, int k, int l, float f, int m, int n, int o, boolean bl) {
        ...
        int p = Mth.ceil((double)((double)f / 2.0));
        int q = Mth.ceil((double)((double)o / 2.0));
        int r = p * 2;
    ...
    private static int getCurrentAirSupplyBubble(int i, int j, int k) {
        return Mth.ceil((float)((float)((i + k) * 10) / (float)j));
    }
    ...
    private void renderSpyglassOverlay(GuiGraphics guiGraphics, float f) {
        ...
        int j = Mth.floor((float)(g * i));
        int k = Mth.floor((float)(h * i));
    ...
    private void updateVignetteBrightness(Entity entity) {
        ...
        float g = Mth.clamp((float)(1.0f - f), (float)0.0f, (float)1.0f);
    ...
    private void renderVignette(GuiGraphics guiGraphics, @Nullable Entity entity) {
        ...
        if (f > 0.0f) {
            f = Mth.clamp((float)f, (float)0.0f, (float)1.0f);
            i = ARGB.colorFromFloat((float)1.0f, (float)0.0f, (float)f, (float)f);
        } else {
            float h = this.vignetteBrightness;
            h = Mth.clamp((float)h, (float)0.0f, (float)1.0f);
            i = ARGB.colorFromFloat((float)1.0f, (float)h, (float)h, (float)h);
        }
    ...
    private void renderConfusionOverlay(GuiGraphics guiGraphics, float f) {
        ...
        guiGraphics.pose().pushMatrix();
        float g = Mth.lerp((float)f, (float)2.0f, (float)1.0f);
        ...
        float k = 0.4f * f;
        float l = 0.2f * f;
        guiGraphics.blit(RenderPipelines.GUI_NAUSEA_OVERLAY, NAUSEA_LOCATION, 0, 0, 0.0f, 0.0f, i, j, i, j, ARGB.colorFromFloat((float)1.0f, (float)h, (float)k, (float)l));
        guiGraphics.pose().popMatrix();
    }
    ...
    private void renderSlot(GuiGraphics guiGraphics, int i, int j, DeltaTracker deltaTracker, Player player, ItemStack itemStack, int k) {
        ...
        guiGraphics.renderItem((LivingEntity)player, itemStack, i, j, k);
        if (f > 0.0f) {
            guiGraphics.pose().popMatrix();
        }
    ...
    private void tick() {
        ...
        if (this.minecraft.player != null) {
            ...
            } else if (this.lastToolHighlight.isEmpty() || !itemStack.is(this.lastToolHighlight.getItem()) || !itemStack.getHoverName().equals((Object)this.lastToolHighlight.getHoverName())) {
    ...
    private void tickAutosaveIndicator() {
        ...
        this.autosaveIndicatorValue = Mth.lerp((float)0.2f, (float)this.autosaveIndicatorValue, (float)(bl ? 1.0f : 0.0f));
    }

    public void setNowPlaying(Component component) {
        MutableComponent component2 = Component.translatable((String)"record.nowPlaying", (Object[])new Object[]{component});
        this.setOverlayMessage((Component)component2, true);
        this.minecraft.getNarrator().saySystemNow((Component)component2);
    }
