...
extends AbstractWidget.WithInactiveMessage {
        ...
        if (this.active) {
            if (this.isFocused()) {
                if (this.canChangeValue) {
                    ...
                    narrationElementOutput.add(NarratedElementType.USAGE, (Component)Component.translatable((String)"narration.slider.usage.focused.keyboard_cannot_change_value"));
                }
            } else {
    ...
    protected void setValue(double d) {
        double e = this.value;
        this.value = Mth.clamp((double)d, (double)0.0, (double)1.0);
        if (e != this.value) {
            this.applyValue();
        }
        this.updateMessage();
