...
implements SoundEventListener {
    ...
    public void render(GuiGraphics guiGraphics) {
        ...
        for (Subtitle subtitle2 : this.audibleSubtitles) {
            ...
            int p = Mth.floor((float)Mth.clampedLerp((float)((float)(Util.getMillis() - soundPlayedAt.time) / (float)(3000.0 * d)), (float)255.0f, (float)75.0f));
            ...
            int q = ARGB.color((int)255, (int)p, (int)p, (int)p);
            if (!bl) {
                if (e > 0.0) {
    ...
    public void onPlaySound(SoundInstance soundInstance, WeighedSoundEvents weighedSoundEvents, float f) {
        ...
        if (!this.subtitles.isEmpty()) {
            for (Subtitle subtitle : this.subtitles) {
                if (!subtitle.getText().equals((Object)component)) continue;
    ...
    static class Subtitle {
        ...
        public void refresh(Vec3 vec3) {
            this.playedAt.removeIf(soundPlayedAt -> vec3.equals((Object)soundPlayedAt.location()));
            this.playedAt.add(new SoundPlayedAt(vec3, Util.getMillis()));
        }
        ...
        public boolean isAudibleFrom(Vec3 vec3) {
            ...
            return vec3.closerThan((Position)soundPlayedAt.location, (double)this.range);
        }
