    ...
    public void display(DebugScreenDisplayer debugScreenDisplayer, @Nullable Level level, @Nullable LevelChunk levelChunk, @Nullable LevelChunk levelChunk2) {
        ...
        if (minecraft.level.isInsideBuildHeight(blockPos.getY())) {
            if (SharedConstants.DEBUG_SHOW_SERVER_DEBUG_VALUES && level instanceof ServerLevel) {
                debugScreenDisplayer.addToGroup(GROUP, List.of("Biome: " + DebugEntryBiome.printBiome((Holder<Biome>)minecraft.level.getBiome(blockPos)), "Server Biome: " + DebugEntryBiome.printBiome((Holder<Biome>)level.getBiome(blockPos))));
            } else {
                debugScreenDisplayer.addLine("Biome: " + DebugEntryBiome.printBiome((Holder<Biome>)minecraft.level.getBiome(blockPos)));
            }
        }
    }
