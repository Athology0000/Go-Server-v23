...
implements Toast {
    ...
    private boolean isChallengeAdvancement() {
        ...
        return optional.isPresent() && ((DisplayInfo)optional.get()).getType().equals((Object)AdvancementType.CHALLENGE);
    }
    ...
    public void render(GuiGraphics guiGraphics, Font font, long l) {
        ...
        List<FormattedCharSequence> list = font.split((FormattedText)displayInfo.getTitle(), 125);
        ...
        if (list.size() == 1) {
            ...
            float f = 300.0f;
            if (l < 1500L) {
                int k = Mth.floor((float)(Mth.clamp((float)((float)(1500L - l) / 300.0f), (float)0.0f, (float)1.0f) * 255.0f));
                guiGraphics.drawString(font, displayInfo.getType().getDisplayName(), 30, 11, ARGB.color((int)k, (int)i), false);
            } else {
                int k = Mth.floor((float)(Mth.clamp((float)((float)(l - 1500L) / 300.0f), (float)0.0f, (float)1.0f) * 252.0f));
                int m = this.height() / 2 - list.size() * font.lineHeight / 2;
                for (FormattedCharSequence formattedCharSequence : list) {
                    guiGraphics.drawString(font, formattedCharSequence, 30, m, ARGB.white((int)k), false);
                    m += font.lineHeight;
                }
            }
        }
