...
extends Screen {
    ...
    protected void init() {
        ...
        this.input = new EditBox(this.minecraft.fontFilterFishy, 4, this.height - 12, this.width - 4, 12, (Component)Component.translatable((String)"chat.editBox")){
    ...
    public boolean mouseScrolled(double d, double e, double f, double g) {
        if (this.commandSuggestions.mouseScrolled(g = Mth.clamp((double)g, (double)-1.0, (double)1.0))) {
            return true;
        }
    ...
    private boolean handleComponentClicked(Style style, boolean bl) {
        ...
        if (bl) {
            ...
            if (clickEvent instanceof ClickEvent.Custom && (custom = (ClickEvent.Custom)clickEvent).id().equals((Object)ChatComponent.QUEUE_EXPAND_ID)) {
    ...
    public void moveInHistory(int i) {
        ...
        if ((j = Mth.clamp((int)j, (int)0, (int)k)) == this.historyPos) {
            return;
        }
        if (j == k) {
            this.historyPos = k;
    ...
    private @Nullable FormattedCharSequence formatChat(String string, int i) {
        if (this.isDraft) {
            return FormattedCharSequence.forward((String)string, (Style)Style.EMPTY.withColor(ChatFormatting.GRAY).withItalic(Boolean.valueOf(true)));
        }
        return null;
    }
    ...
    protected void updateNarrationState(NarrationElementOutput narrationElementOutput) {
        ...
        if (!string.isEmpty()) {
            narrationElementOutput.nest().add(NarratedElementType.TITLE, (Component)Component.translatable((String)"chat_screen.message", (Object[])new Object[]{string}));
        }
    }
    ...
    public String normalizeChatMessage(String string) {
        return StringUtil.trimChatMessage((String)StringUtils.normalizeSpace((String)string.trim()));
    }
