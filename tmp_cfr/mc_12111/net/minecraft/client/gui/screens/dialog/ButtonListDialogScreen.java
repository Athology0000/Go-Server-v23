...
extends DialogScreen<T> {
    ...
    protected void populateBodyElements(LinearLayout linearLayout, DialogControlSet dialogControlSet, T buttonListDialog, DialogConnectionAccess dialogConnectionAccess) {
        ...
        List<Button> list = this.createListActions(buttonListDialog, dialogConnectionAccess).map(actionButton -> dialogControlSet.createActionButton((ActionButton)actionButton).build()).toList();
    ...
    protected void updateHeaderAndFooter(HeaderAndFooterLayout headerAndFooterLayout, DialogControlSet dialogControlSet, T buttonListDialog, DialogConnectionAccess dialogConnectionAccess) {
        ...
        buttonListDialog.exitAction().ifPresentOrElse(actionButton -> headerAndFooterLayout.addToFooter(dialogControlSet.createActionButton((ActionButton)actionButton).build()), () -> headerAndFooterLayout.setFooterHeight(5));
    }
}
