...
extends ButtonListDialogScreen<ServerLinksDialog> {
    ...
    private static ActionButton createDialogClickAction(ServerLinksDialog serverLinksDialog, ServerLinks.Entry entry) {
        return new ActionButton(new CommonButtonData(entry.displayName(), serverLinksDialog.buttonWidth()), Optional.of(new StaticAction((ClickEvent)new ClickEvent.OpenUrl(entry.link()))));
    }
}
