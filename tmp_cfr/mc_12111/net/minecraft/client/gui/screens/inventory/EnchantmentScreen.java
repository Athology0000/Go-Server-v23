...
extends AbstractContainerScreen<EnchantmentMenu> {
    ...
    private static final Identifier ENCHANTING_BOOK_LOCATION = Identifier.withDefaultNamespace((String)"textures/entity/enchanting_table_book.png");
    ...
    public boolean mouseClicked(MouseButtonEvent mouseButtonEvent, boolean bl) {
        ...
        for (int k = 0; k < 3; ++k) {
            ...
            if (!(d >= 0.0) || !(e >= 0.0) || !(d < 108.0) || !(e < 19.0) || !((EnchantmentMenu)this.menu).clickMenuButton((Player)this.minecraft.player, k)) continue;
    ...
    protected void renderBg(GuiGraphics guiGraphics, float f, int i, int j) {
        ...
        for (int n = 0; n < 3; ++n) {
            ...
            if (!(m >= n + 1 && this.minecraft.player.experienceLevel >= q || this.minecraft.player.hasInfiniteMaterials())) {
                ...
                guiGraphics.drawWordWrap(this.font, formattedText, p, l + 16 + 19 * n, r, ARGB.opaque((int)((s & 0xFEFEFE) >> 1)), false);
                s = -12550384;
            } else {
                int t = i - (k + 60);
                int u = j - (l + 14 + 19 * n);
    ...
    private void renderBook(GuiGraphics guiGraphics, int i, int j) {
        ...
        float g = Mth.lerp((float)f, (float)this.oOpen, (float)this.open);
        float h = Mth.lerp((float)f, (float)this.oFlip, (float)this.flip);
        int k = i + 14;
        int l = j + 14;
        int m = k + 38;
        int n = l + 31;
