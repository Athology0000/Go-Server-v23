...
public class WorldCreationUiState {
    ...
    public WorldCreationUiState(Path path, WorldCreationContext worldCreationContext, Optional<ResourceKey<WorldPreset>> optional, OptionalLong optionalLong) {
        ...
        this.worldType = new WorldTypeEntry((Holder<WorldPreset>)((Holder)WorldCreationUiState.findPreset(worldCreationContext, optional).orElse(null)));
    ...
    private String findResultFolder(String string) {
        ...
        try {
            return FileUtil.findAvailableName((Path)this.savesFolder, (String)(!string2.isEmpty() ? string2 : DEFAULT_WORLD_NAME.getString()), (String)"");
        }
    ...
    public void setSeed(String string) {
        this.seed = string;
        this.settings = this.settings.withOptions(worldOptions -> worldOptions.withSeed(WorldOptions.parseSeed((String)this.getSeed())));
        this.onChanged();
    }
    ...
    protected boolean tryUpdateDataConfiguration(WorldDataConfiguration worldDataConfiguration) {
        ...
        if (worldDataConfiguration2.dataPacks().getEnabled().equals(worldDataConfiguration.dataPacks().getEnabled()) && worldDataConfiguration2.enabledFeatures().equals((Object)worldDataConfiguration.enabledFeatures())) {
    ...
    private void updatePresetLists() {
        Registry registry = this.getSettings().worldgenLoadContext().lookupOrThrow(Registries.WORLD_PRESET);
        this.normalPresetList.clear();
        this.normalPresetList.addAll(WorldCreationUiState.getNonEmptyList((Registry<WorldPreset>)registry, (TagKey<WorldPreset>)WorldPresetTags.NORMAL).orElseGet(() -> registry.listElements().map(WorldTypeEntry::new).toList()));
        this.altPresetList.clear();
        this.altPresetList.addAll((Collection<WorldTypeEntry>)WorldCreationUiState.getNonEmptyList((Registry<WorldPreset>)registry, (TagKey<WorldPreset>)WorldPresetTags.EXTENDED).orElse(this.normalPresetList));
    ...
    public static enum SelectedGameMode {
        ...
        private SelectedGameMode(String string2, GameType gameType) {
            ...
            this.displayName = Component.translatable((String)("selectWorld.gameMode." + string2));
            this.info = Component.translatable((String)("selectWorld.gameMode." + string2 + ".info"));
        }
    ...
    public record WorldTypeEntry(@Nullable Holder<WorldPreset> preset) {
        ...
        public Component describePreset() {
            return Optional.ofNullable(this.preset).flatMap(Holder::unwrapKey).map(resourceKey -> Component.translatable((String)resourceKey.identifier().toLanguageKey("generator"))).orElse(CUSTOM_WORLD_DESCRIPTION);
        }
