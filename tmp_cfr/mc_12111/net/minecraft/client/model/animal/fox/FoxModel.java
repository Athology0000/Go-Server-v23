...
extends EntityModel<FoxRenderState> {
    ...
    public void setupAnim(FoxRenderState foxRenderState) {
        ...
        this.rightHindLeg.xRot = Mth.cos((double)(g * 0.6662f)) * 1.4f * f;
        this.leftHindLeg.xRot = Mth.cos((double)(g * 0.6662f + (float)Math.PI)) * 1.4f * f;
        this.rightFrontLeg.xRot = Mth.cos((double)(g * 0.6662f + (float)Math.PI)) * 1.4f * f;
        this.leftFrontLeg.xRot = Mth.cos((double)(g * 0.6662f)) * 1.4f * f;
        ...
        if (foxRenderState.isSleeping) {
            this.head.xRot = 0.0f;
            this.head.yRot = -2.0943952f;
            this.head.zRot = Mth.cos((double)(foxRenderState.ageInTicks * 0.027f)) / 22.0f;
        }
        if (foxRenderState.isCrouching) {
            this.body.yRot = i = Mth.cos((double)foxRenderState.ageInTicks) * 0.01f;
