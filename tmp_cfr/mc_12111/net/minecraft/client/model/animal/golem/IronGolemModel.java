...
extends EntityModel<IronGolemRenderState> {
    ...
    public void setupAnim(IronGolemRenderState ironGolemRenderState) {
        ...
        if (f > 0.0f) {
            ...
            if (i > 0) {
                this.rightArm.xRot = -0.8f + 0.025f * Mth.triangleWave((float)i, (float)70.0f);
                this.leftArm.xRot = 0.0f;
            } else {
                this.rightArm.xRot = (-0.2f + 1.5f * Mth.triangleWave((float)h, (float)13.0f)) * g;
                this.leftArm.xRot = (-0.2f - 1.5f * Mth.triangleWave((float)h, (float)13.0f)) * g;
            }
        }
        this.head.yRot = ironGolemRenderState.yRot * ((float)Math.PI / 180);
        this.head.xRot = ironGolemRenderState.xRot * ((float)Math.PI / 180);
        this.rightLeg.xRot = -1.5f * Mth.triangleWave((float)h, (float)13.0f) * g;
        this.leftLeg.xRot = 1.5f * Mth.triangleWave((float)h, (float)13.0f) * g;
