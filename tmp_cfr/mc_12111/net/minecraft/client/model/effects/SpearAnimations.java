...
public class SpearAnimations {
    static float progress(float f, float g, float h) {
        return Mth.clamp((float)Mth.inverseLerp((float)f, (float)g, (float)h), (float)0.0f, (float)1.0f);
    }
    ...
    public static <T extends HumanoidRenderState> void thirdPersonHandUse(ModelPart modelPart, ModelPart modelPart2, boolean bl, ItemStack itemStack, T humanoidRenderState) {
        ...
        modelPart.yRot = (float)Math.PI / 180 * Math.clamp(57.295776f * modelPart.yRot, -60.0f, 60.0f);
        modelPart.xRot = (float)Math.PI / 180 * Math.clamp(57.295776f * modelPart.xRot, -120.0f, 30.0f);
    ...
    public static <S extends ArmedEntityRenderState> void thirdPersonUseItem(S armedEntityRenderState, PoseStack poseStack, float f, HumanoidArm humanoidArm, ItemStack itemStack) {
        ...
        float g = Ease.inQuad((float)SpearAnimations.progress(armedEntityRenderState.attackTime, 0.05f, 0.2f));
        float h = Ease.inOutExpo((float)SpearAnimations.progress(armedEntityRenderState.attackTime, 0.4f, 1.0f));
        UseParams useParams = UseParams.fromKineticWeapon(kineticWeapon, f);
        int i = humanoidArm == HumanoidArm.RIGHT ? 1 : -1;
        float j = 1.0f - Ease.outBack((float)(1.0f - useParams.raiseProgress()));
        float k = 0.125f;
    ...
    public static <T extends HumanoidRenderState> void thirdPersonAttackHand(HumanoidModel<T> humanoidModel, T humanoidRenderState) {
        ...
        float g = Ease.inOutSine((float)SpearAnimations.progress(f, 0.0f, 0.05f));
        float h = Ease.inQuad((float)SpearAnimations.progress(f, 0.05f, 0.2f));
        float i = Ease.inOutExpo((float)SpearAnimations.progress(f, 0.4f, 1.0f));
    ...
    public static <S extends ArmedEntityRenderState> void thirdPersonAttackItem(S armedEntityRenderState, PoseStack poseStack) {
        ...
        float i = Ease.inQuad((float)SpearAnimations.progress(h, 0.05f, 0.2f));
        float j = Ease.inOutExpo((float)SpearAnimations.progress(h, 0.4f, 1.0f));
    ...
    private static float hitFeedbackAmount(float f) {
        return 0.4f * (Ease.outQuart((float)SpearAnimations.progress(f, 1.0f, 3.0f)) - Ease.inOutSine((float)SpearAnimations.progress(f, 3.0f, 10.0f)));
    }
    ...
    public static void firstPersonUse(float f, PoseStack poseStack, float g, HumanoidArm humanoidArm, ItemStack itemStack) {
        ...
        poseStack.rotateAround((Quaternionfc)Axis.XP.rotationDegrees(-65.0f * Ease.inOutBack((float)useParams.raiseProgress()) - 35.0f * useParams.lowerProgress() + 100.0f * useParams.raiseBackProgress() + -0.5f * useParams.swayScaleFast()), 0.0f, 0.1f, 0.0f);
    ...
    public static void firstPersonAttack(float f, PoseStack poseStack, int i, HumanoidArm humanoidArm) {
        float g = Ease.inOutSine((float)SpearAnimations.progress(f, 0.0f, 0.05f));
        float h = Ease.outBack((float)SpearAnimations.progress(f, 0.05f, 0.2f));
        float j = Ease.inOutExpo((float)SpearAnimations.progress(f, 0.4f, 1.0f));
    ...
    record UseParams(float raiseProgress, float raiseProgressStart, float raiseProgressMiddle, float raiseProgressEnd, float swayProgress, float lowerProgress, float raiseBackProgress, float swayIntensity, float swayScaleSlow, float swayScaleFast) {
        public static UseParams fromKineticWeapon(KineticWeapon kineticWeapon, float f) {
            ...
            float r = Ease.outCubic((float)Ease.inOutElastic((float)SpearAnimations.progress(f - 20.0f, m, l)));
            float s = SpearAnimations.progress(f, n - 5, n);
            float t = 2.0f * Ease.outCirc((float)q) - 2.0f * Ease.inCirc((float)s);
            float u = Mth.sin((double)(f * 19.0f * ((float)Math.PI / 180))) * t;
            float v = Mth.sin((double)(f * 30.0f * ((float)Math.PI / 180))) * t;
