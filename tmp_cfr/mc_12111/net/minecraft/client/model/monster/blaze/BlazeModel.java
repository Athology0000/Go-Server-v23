...
extends EntityModel<LivingEntityRenderState> {
    ...
    public static LayerDefinition createBodyLayer() {
        ...
        for (i = 0; i < 4; ++i) {
            g = Mth.cos((double)f) * 9.0f;
            h = -2.0f + Mth.cos((double)((float)(i * 2) * 0.25f));
            j = Mth.sin((double)f) * 9.0f;
            partDefinition.addOrReplaceChild(BlazeModel.getPartName(i), cubeListBuilder, PartPose.offset(g, h, j));
            f += 1.5707964f;
        }
        f = 0.7853982f;
        for (i = 4; i < 8; ++i) {
            g = Mth.cos((double)f) * 7.0f;
            h = 2.0f + Mth.cos((double)((float)(i * 2) * 0.25f));
            j = Mth.sin((double)f) * 7.0f;
            partDefinition.addOrReplaceChild(BlazeModel.getPartName(i), cubeListBuilder, PartPose.offset(g, h, j));
            f += 1.5707964f;
        }
        f = 0.47123894f;
        for (i = 8; i < 12; ++i) {
            g = Mth.cos((double)f) * 5.0f;
            h = 11.0f + Mth.cos((double)((float)i * 1.5f * 0.5f));
            j = Mth.sin((double)f) * 5.0f;
            partDefinition.addOrReplaceChild(BlazeModel.getPartName(i), cubeListBuilder, PartPose.offset(g, h, j));
            f += 1.5707964f;
        }
    ...
    public void setupAnim(LivingEntityRenderState livingEntityRenderState) {
        ...
        for (i = 0; i < 4; ++i) {
            this.upperBodyParts[i].y = -2.0f + Mth.cos((double)(((float)(i * 2) + livingEntityRenderState.ageInTicks) * 0.25f));
            this.upperBodyParts[i].x = Mth.cos((double)f) * 9.0f;
            this.upperBodyParts[i].z = Mth.sin((double)f) * 9.0f;
            f += 1.5707964f;
        }
        f = 0.7853982f + livingEntityRenderState.ageInTicks * (float)Math.PI * 0.03f;
        for (i = 4; i < 8; ++i) {
            this.upperBodyParts[i].y = 2.0f + Mth.cos((double)(((float)(i * 2) + livingEntityRenderState.ageInTicks) * 0.25f));
            this.upperBodyParts[i].x = Mth.cos((double)f) * 7.0f;
            this.upperBodyParts[i].z = Mth.sin((double)f) * 7.0f;
            f += 1.5707964f;
        }
        f = 0.47123894f + livingEntityRenderState.ageInTicks * (float)Math.PI * -0.05f;
        for (i = 8; i < 12; ++i) {
            this.upperBodyParts[i].y = 11.0f + Mth.cos((double)(((float)i * 1.5f + livingEntityRenderState.ageInTicks) * 0.5f));
            this.upperBodyParts[i].x = Mth.cos((double)f) * 5.0f;
            this.upperBodyParts[i].z = Mth.sin((double)f) * 5.0f;
            f += 1.5707964f;
        }
