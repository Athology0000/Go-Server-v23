...
extends EntityModel<EnderDragonRenderState> {
    ...
    public void setupAnim(EnderDragonRenderState enderDragonRenderState) {
        ...
        float l = Mth.wrapDegrees((float)(enderDragonRenderState.getHistoricalPos(5).yRot() - enderDragonRenderState.getHistoricalPos(10).yRot()));
        float m = Mth.wrapDegrees((float)(enderDragonRenderState.getHistoricalPos(5).yRot() + l / 2.0f));
        for (int n = 0; n < 5; ++n) {
            ...
            modelPart.yRot = Mth.wrapDegrees((float)(sample2.yRot() - sample.yRot())) * ((float)Math.PI / 180) * 1.5f;
            modelPart.xRot = o + enderDragonRenderState.getHeadPartYOffset(n, sample, sample2) * ((float)Math.PI / 180) * 1.5f * 5.0f;
            modelPart.zRot = -Mth.wrapDegrees((float)(sample2.yRot() - m)) * ((float)Math.PI / 180) * 1.5f;
            modelPart.y = i;
            modelPart.z = j;
            modelPart.x = h;
        ...
        this.head.yRot = Mth.wrapDegrees((float)(sample3.yRot() - sample.yRot())) * ((float)Math.PI / 180);
        this.head.xRot = Mth.wrapDegrees((float)enderDragonRenderState.getHeadPartYOffset(6, sample, sample3)) * ((float)Math.PI / 180) * 1.5f * 5.0f;
        this.head.zRot = -Mth.wrapDegrees((float)(sample3.yRot() - m)) * ((float)Math.PI / 180);
        ...
        for (int q = 0; q < 12; ++q) {
            ...
            modelPart2.yRot = (Mth.wrapDegrees((float)(sample4.yRot() - sample.yRot())) * 1.5f + 180.0f) * ((float)Math.PI / 180);
            modelPart2.xRot = (p += Mth.sin((double)((float)q * 0.45f + f)) * 0.05f) + (float)(sample4.y() - sample.y()) * ((float)Math.PI / 180) * 1.5f * 5.0f;
            modelPart2.zRot = Mth.wrapDegrees((float)(sample4.yRot() - m)) * ((float)Math.PI / 180) * 1.5f;
            modelPart2.y = i;
            modelPart2.z = j;
            modelPart2.x = h;
