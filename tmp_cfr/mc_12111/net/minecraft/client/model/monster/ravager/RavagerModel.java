...
extends EntityModel<RavagerRenderState> {
    ...
    public void setupAnim(RavagerRenderState ravagerRenderState) {
        ...
        int i = 10;
        if (g > 0.0f) {
            h = Mth.triangleWave((float)g, (float)10.0f);
            j = (1.0f + h) * 0.5f;
            float k = j * j * j * 12.0f;
            float l = k * Mth.sin((double)this.neck.xRot);
            this.neck.z = -6.5f + k;
            this.neck.y = -7.0f - l;
            this.mouth.xRot = g > 5.0f ? Mth.sin((double)((-4.0f + g) / 4.0f)) * (float)Math.PI * 0.4f : 0.15707964f * Mth.sin((double)((float)Math.PI * g / 10.0f));
        } else {
            h = -1.0f;
            j = -1.0f * Mth.sin((double)this.neck.xRot);
            this.neck.x = 0.0f;
            this.neck.y = -7.0f - j;
            this.neck.z = 5.5f;
            boolean bl = f > 0.0f;
            ...
            if (bl) {
                ...
                float l = Mth.sin((double)(ravagerRenderState.roarAnimation * (float)Math.PI * 0.25f));
        ...
        this.rightHindLeg.xRot = Mth.cos((double)(h * 0.6662f)) * j;
        this.leftHindLeg.xRot = Mth.cos((double)(h * 0.6662f + (float)Math.PI)) * j;
        this.rightFrontLeg.xRot = Mth.cos((double)(h * 0.6662f + (float)Math.PI)) * j;
        this.leftFrontLeg.xRot = Mth.cos((double)(h * 0.6662f)) * j;
    }
}
