...
extends ChunkSource {
    ...
    public ClientChunkCache(ClientLevel clientLevel, int i) {
        ...
        this.emptyChunk = new EmptyLevelChunk((Level)clientLevel, new ChunkPos(0, 0), (Holder)clientLevel.registryAccess().lookupOrThrow(Registries.BIOME).getOrThrow(Biomes.PLAINS));
        this.lightEngine = new LevelLightEngine((LightChunkGetter)this, true, clientLevel.dimensionType().hasSkyLight());
    ...
    final class Storage {
        ...
        public void onSectionEmptinessChanged(int i, int j, int k, boolean bl) {
            ...
            long l = SectionPos.asLong((int)i, (int)j, (int)k);
            if (bl) {
        ...
        private void dropEmptySections(LevelChunk levelChunk) {
            ...
            for (int i = 0; i < levelChunkSections.length; ++i) {
                ChunkPos chunkPos = levelChunk.getPos();
                this.loadedEmptySections.remove(SectionPos.asLong((int)chunkPos.x, (int)levelChunk.getSectionYFromSectionIndex(i), (int)chunkPos.z));
            }
        }
        ...
        private void addEmptySections(LevelChunk levelChunk) {
            ...
            for (int i = 0; i < levelChunkSections.length; ++i) {
                ...
                ChunkPos chunkPos = levelChunk.getPos();
                this.loadedEmptySections.add(SectionPos.asLong((int)chunkPos.x, (int)levelChunk.getSectionYFromSectionIndex(i), (int)chunkPos.z));
            }
        }
        ...
        void refreshEmptySections(LevelChunk levelChunk) {
            ...
            for (int i = 0; i < levelChunkSections.length; ++i) {
                LevelChunkSection levelChunkSection = levelChunkSections[i];
                long l = SectionPos.asLong((int)chunkPos.x, (int)levelChunk.getSectionYFromSectionIndex(i), (int)chunkPos.z);
