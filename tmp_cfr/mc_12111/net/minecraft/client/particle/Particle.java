...
public abstract class Particle {
    ...
    public void move(double d, double e, double f) {
        ...
        if (this.hasPhysics && (d != 0.0 || e != 0.0 || f != 0.0) && d * d + e * e + f * f < MAXIMUM_COLLISION_VELOCITY_SQUARED) {
            Vec3 vec3 = Entity.collideBoundingBox(null, (Vec3)new Vec3(d, e, f), (AABB)this.getBoundingBox(), (Level)this.level, List.of());
            d = vec3.x;
            e = vec3.y;
            f = vec3.z;
        }
    ...
    protected int getLightColor(float f) {
        ...
        if (this.level.hasChunkAt(blockPos)) {
            return LevelRenderer.getLightColor((BlockAndTintGetter)this.level, blockPos);
        }
        return 0;
    }
    ...
    public record LifetimeAlpha(float startAlpha, float endAlpha, float startAtNormalizedAge, float endAtNormalizedAge) {
        ...
        public float currentAlphaForAge(int i, int j, float f) {
            ...
            float g = Mth.inverseLerp((float)(((float)i + f) / (float)j), (float)this.startAtNormalizedAge, (float)this.endAtNormalizedAge);
            return Mth.clampedLerp((float)g, (float)this.startAlpha, (float)this.endAlpha);
        }
    }
}
