...
extends SingleQuadParticle {
    ...
    TrailParticle(ClientLevel clientLevel, double d, double e, double f, double g, double h, double i, Vec3 vec3, int j, TextureAtlasSprite textureAtlasSprite) {
        ...
        j = ARGB.scaleRGB((int)j, (float)(0.875f + this.random.nextFloat() * 0.25f), (float)(0.875f + this.random.nextFloat() * 0.25f), (float)(0.875f + this.random.nextFloat() * 0.25f));
        this.rCol = (float)ARGB.red((int)j) / 255.0f;
        this.gCol = (float)ARGB.green((int)j) / 255.0f;
        this.bCol = (float)ARGB.blue((int)j) / 255.0f;
        this.quadSize = 0.26f;
        this.target = vec3;
    }
    ...
    public void tick() {
        ...
        this.x = Mth.lerp((double)d, (double)this.x, (double)this.target.x());
        this.y = Mth.lerp((double)d, (double)this.y, (double)this.target.y());
        this.z = Mth.lerp((double)d, (double)this.z, (double)this.target.z());
    }
