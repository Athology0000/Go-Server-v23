...
implements ClientAvatarEntity {
    ...
    public AbstractClientPlayer(ClientLevel clientLevel, GameProfile gameProfile) {
        super((Level)clientLevel, gameProfile);
    ...
    public @Nullable Component belowNameDisplay() {
        ...
        if (objective != null) {
            ReadOnlyScoreInfo readOnlyScoreInfo = scoreboard.getPlayerScoreInfo((ScoreHolder)this, objective);
            MutableComponent component = ReadOnlyScoreInfo.safeFormatValue((ReadOnlyScoreInfo)readOnlyScoreInfo, (NumberFormat)objective.numberFormatOrDefault((NumberFormat)StyledFormat.NO_STYLE));
            return Component.empty().append((Component)component).append(CommonComponents.SPACE).append(objective.getDisplayName());
        }
        return null;
    }
    ...
    public // Could not load outer class - annotation placement on inner may be incorrect
     @Nullable Parrot.Variant getParrotVariantOnShoulder(boolean bl) {
    ...
    public float getFieldOfViewModifier(boolean bl, float f) {
        ...
        if (this.isUsingItem()) {
            if (this.getUseItem().is(Items.BOW)) {
                ...
                g *= 1.0f - Mth.square((float)i) * 0.15f;
            } else if (bl && this.isScoping()) {
                return 0.1f;
            }
        }
        return Mth.lerp((float)f, (float)1.0f, (float)g);
    }
