...
public class ItemInHandRenderer {
    ...
    public void renderItem(LivingEntity livingEntity, ItemStack itemStack, ItemDisplayContext itemDisplayContext, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int i) {
        ...
        this.itemModelResolver.updateForTopItem(itemStackRenderState, itemStack, itemDisplayContext, livingEntity.level(), (ItemOwner)livingEntity, livingEntity.getId() + itemDisplayContext.ordinal());
    ...
    private float calculateMapTilt(float f) {
        ...
        g = -Mth.cos((double)(g * (float)Math.PI)) * 0.5f + 0.5f;
        return g;
    }
    ...
    private void renderOneHandedMap(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int i, float f, HumanoidArm humanoidArm, float g, ItemStack itemStack) {
        ...
        float j = Mth.sqrt((float)g);
        float k = Mth.sin((double)(j * (float)Math.PI));
        float l = -0.5f * k;
        float m = 0.4f * Mth.sin((double)(j * ((float)Math.PI * 2)));
        float n = -0.3f * Mth.sin((double)(g * (float)Math.PI));
    ...
    private void renderTwoHandedMap(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int i, float f, float g, float h) {
        float j = Mth.sqrt((float)h);
        float k = -0.2f * Mth.sin((double)(h * (float)Math.PI));
        float l = -0.4f * Mth.sin((double)(j * (float)Math.PI));
        ...
        float n = Mth.sin((double)(j * (float)Math.PI));
    ...
    private void renderPlayerArm(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int i, float f, float g, HumanoidArm humanoidArm) {
        ...
        float j = Mth.sqrt((float)g);
        float k = -0.3f * Mth.sin((double)(j * (float)Math.PI));
        float l = 0.4f * Mth.sin((double)(j * ((float)Math.PI * 2)));
        float m = -0.4f * Mth.sin((double)(g * (float)Math.PI));
        ...
        float n = Mth.sin((double)(g * g * (float)Math.PI));
        float o = Mth.sin((double)(j * (float)Math.PI));
    ...
    private void applyEatTransform(PoseStack poseStack, float f, HumanoidArm humanoidArm, ItemStack itemStack, Player player) {
        ...
        float h = g / (float)itemStack.getUseDuration((LivingEntity)player);
        if (h < 0.8f) {
            i = Mth.abs((float)(Mth.cos((double)(g / 4.0f * (float)Math.PI)) * 0.1f));
    ...
    private void applyBrushTransform(PoseStack poseStack, float f, HumanoidArm humanoidArm, Player player) {
        ...
        float m = -15.0f;
        int n = 2;
        float o = -15.0f + 75.0f * Mth.cos((double)(i * 2.0f * (float)Math.PI));
        if (humanoidArm != HumanoidArm.RIGHT) {
    ...
    private void applyItemArmAttackTransform(PoseStack poseStack, HumanoidArm humanoidArm, float f) {
        ...
        float g = Mth.sin((double)(f * f * (float)Math.PI));
        poseStack.mulPose((Quaternionfc)Axis.YP.rotationDegrees((float)i * (45.0f + g * -20.0f)));
        float h = Mth.sin((double)(Mth.sqrt((float)f) * (float)Math.PI));
    ...
    public void renderHandsWithItems(float f, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, LocalPlayer localPlayer, int i) {
        ...
        InteractionHand interactionHand = (InteractionHand)MoreObjects.firstNonNull((Object)localPlayer.swingingArm, (Object)InteractionHand.MAIN_HAND);
        ...
        float j = Mth.lerp((float)f, (float)localPlayer.xBobO, (float)localPlayer.xBob);
        float k = Mth.lerp((float)f, (float)localPlayer.yBobO, (float)localPlayer.yBob);
        ...
        if (handRenderSelection.renderMainHand) {
            ...
            m = this.itemModelResolver.swapAnimationScale(this.mainHandItem) * (1.0f - Mth.lerp((float)f, (float)this.oMainHandHeight, (float)this.mainHandHeight));
        ...
        if (handRenderSelection.renderOffHand) {
            ...
            m = this.itemModelResolver.swapAnimationScale(this.offHandItem) * (1.0f - Mth.lerp((float)f, (float)this.oOffHandHeight, (float)this.offHandHeight));
    ...
    private static boolean isChargedCrossbow(ItemStack itemStack) {
        return itemStack.is(Items.CROSSBOW) && CrossbowItem.isCharged((ItemStack)itemStack);
    }
    ...
    private void renderArmWithItem(AbstractClientPlayer abstractClientPlayer, float f, float g, InteractionHand interactionHand, float h, ItemStack itemStack, float i, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int j) {
        ...
        if (itemStack.isEmpty()) {
            ...
            boolean bl2 = CrossbowItem.isCharged((ItemStack)itemStack);
            ...
            if (abstractClientPlayer.isUsingItem() && abstractClientPlayer.getUseItemRemainingTicks() > 0 && abstractClientPlayer.getUsedItemHand() == interactionHand && !bl2) {
                ...
                if (m > 0.1f) {
                    float n2 = Mth.sin((double)((l - 0.1f) * 1.3f));
                    float o = m - 0.1f;
                    float p = n2 * o;
            ...
            this.renderItem((LivingEntity)abstractClientPlayer, itemStack, bl3 ? ItemDisplayContext.FIRST_PERSON_RIGHT_HAND : ItemDisplayContext.FIRST_PERSON_LEFT_HAND, poseStack, submitNodeCollector, j);
        } else {
            int q;
            ...
            if (abstractClientPlayer.isUsingItem() && abstractClientPlayer.getUseItemRemainingTicks() > 0 && abstractClientPlayer.getUsedItemHand() == interactionHand) {
                ...
                switch (itemUseAnimation) {
                    ...
                    case BOW: {
                        ...
                        if (m > 0.1f) {
                            float n3 = Mth.sin((double)((l - 0.1f) * 1.3f));
                            float o = m - 0.1f;
                            float p = n3 * o;
                            poseStack.translate(p * 0.0f, p * 0.004f, p * 0.0f);
                        }
                    ...
                    case TRIDENT: {
                        ...
                        if (m > 0.1f) {
                            float n4 = Mth.sin((double)((l - 0.1f) * 1.3f));
                            float o = m - 0.1f;
                            float p = n4 * o;
                            poseStack.translate(p * 0.0f, p * 0.004f, p * 0.0f);
                        }
            ...
            this.renderItem((LivingEntity)abstractClientPlayer, itemStack, bl2 ? ItemDisplayContext.FIRST_PERSON_RIGHT_HAND : ItemDisplayContext.FIRST_PERSON_LEFT_HAND, poseStack, submitNodeCollector, j);
        }
        poseStack.popPose();
    }

    private void swingArm(float f, PoseStack poseStack, int i, HumanoidArm humanoidArm) {
        float g = -0.4f * Mth.sin((double)(Mth.sqrt((float)f) * (float)Math.PI));
        float h = 0.2f * Mth.sin((double)(Mth.sqrt((float)f) * ((float)Math.PI * 2)));
        float j = -0.2f * Mth.sin((double)(f * (float)Math.PI));
    ...
    private boolean shouldInstantlyReplaceVisibleItem(ItemStack itemStack, ItemStack itemStack2) {
        if (ItemStack.matchesIgnoringComponents((ItemStack)itemStack, (ItemStack)itemStack2, DataComponentType::ignoreSwapAnimation)) {
            return true;
        }
    ...
    public void tick() {
        ...
        if (localPlayer.isHandsBusy()) {
            ...
            this.mainHandHeight += Mth.clamp((float)(g - this.mainHandHeight), (float)-0.4f, (float)0.4f);
            this.offHandHeight += Mth.clamp((float)(h - this.offHandHeight), (float)-0.4f, (float)0.4f);
        }
