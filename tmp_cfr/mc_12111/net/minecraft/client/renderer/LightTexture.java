...
implements AutoCloseable {
    ...
    private float calculateDarknessScale(LivingEntity livingEntity, float f, float g) {
        float h = 0.45f * f;
        return Math.max(0.0f, Mth.cos((double)(((float)livingEntity.tickCount - g) * (float)Math.PI * 0.025f)) * h);
    }
    ...
    public void updateLightTexture(float f) {
        ...
        float h = ((Float)camera.attributeProbe().getValue(EnvironmentAttributes.SKY_LIGHT_FACTOR, f)).floatValue();
        ...
        float l = this.calculateDarknessScale((LivingEntity)this.minecraft.player, k, f) * j;
        float m = this.minecraft.player.getWaterVision();
        float n = this.minecraft.player.hasEffect(MobEffects.NIGHT_VISION) ? GameRenderer.getNightVisionScale((LivingEntity)this.minecraft.player, f) : (m > 0.0f && this.minecraft.player.hasEffect(MobEffects.CONDUIT_POWER) ? m : 0.0f);
        ...
        try (GpuBuffer.MappedView mappedView = commandEncoder.mapBuffer(this.ubo.currentBuffer(), false, true);){
            Std140Builder.intoBuffer(mappedView.data()).putFloat(g).putFloat(h).putFloat(o).putFloat(n).putFloat(l).putFloat(this.renderer.getDarkenWorldAmount(f)).putFloat(Math.max(0.0f, p - k)).putVec3((Vector3fc)ARGB.vector3fFromRGB24((int)i)).putVec3((Vector3fc)vector3f);
        }
    ...
    public static float getBrightness(float f, int i) {
        ...
        return Mth.lerp((float)f, (float)h, (float)1.0f);
    }
