...
implements AutoCloseable {
    ...
    public void addToFrame(FrameGraphBuilder frameGraphBuilder, Map<Identifier, ResourceHandle<RenderTarget>> map, GpuBufferSlice gpuBufferSlice) {
        ...
        framePass.executes(() -> {
            ...
            List<InputTexture> list = this.inputs.stream().map(input -> new InputTexture(input.samplerName(), input.texture(map), samplerCache.getClampToEdge(input.bilinear() ? FilterMode.LINEAR : FilterMode.NEAREST))).toList();
            try (GpuBuffer.MappedView mappedView = commandEncoder.mapBuffer(this.infoUbo.currentBuffer(), false, true);){
                Iterator<InputTexture> std140Builder = Std140Builder.intoBuffer(mappedView.data());
