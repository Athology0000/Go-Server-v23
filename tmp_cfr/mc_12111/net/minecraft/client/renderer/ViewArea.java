...
public class ViewArea {
    ...
    public ViewArea(SectionRenderDispatcher sectionRenderDispatcher, Level level, int i, LevelRenderer levelRenderer) {
        ...
        this.cameraSectionPos = SectionPos.of((int)(this.viewDistance + 1), (int)0, (int)(this.viewDistance + 1));
    }
        ...
        for (int j = 0; j < this.sectionGridSizeX; ++j) {
            for (int k = 0; k < this.sectionGridSizeY; ++k) {
                for (int l = 0; l < this.sectionGridSizeZ; ++l) {
                    ...
                    this.sections[m] = new SectionRenderDispatcher.RenderSection(sectionRenderDispatcher2, m, SectionPos.asLong((int)j, (int)(k + this.level.getMinSectionY()), (int)l));
                }
            }
        }
    }
        ...
        for (int i = 0; i < this.sectionGridSizeX; ++i) {
            ...
            for (int l = 0; l < this.sectionGridSizeZ; ++l) {
                ...
                for (int o = 0; o < this.sectionGridSizeY; ++o) {
                    ...
                    if (q == SectionPos.asLong((int)k, (int)p, (int)n)) continue;
                    renderSection.setSectionNode(SectionPos.asLong((int)k, (int)p, (int)n));
                }
            }
        }
    ...
    protected @Nullable SectionRenderDispatcher.RenderSection getRenderSectionAt(BlockPos blockPos) {
        return this.getRenderSection(SectionPos.asLong((BlockPos)blockPos));
    }

    protected @Nullable SectionRenderDispatcher.RenderSection getRenderSection(long l) {
        int i = SectionPos.x((long)l);
        int j = SectionPos.y((long)l);
        int k = SectionPos.z((long)l);
