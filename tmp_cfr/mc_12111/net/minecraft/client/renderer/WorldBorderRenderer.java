...
public class WorldBorderRenderer {
    ...
    private void rebuildWorldBorderBuffer(WorldBorderRenderState worldBorderRenderState, double d, double e, double f, float g, float h, float i) {
        try (ByteBufferBuilder byteBufferBuilder = ByteBufferBuilder.exactlySized(DefaultVertexFormat.POSITION_TEX.getVertexSize() * 4 * 4);){
            ...
            double n = Math.max((double)Mth.floor((double)(e - d)), l);
            double o = Math.min((double)Mth.ceil((double)(e + d)), m);
            float p = (float)(Mth.floor((double)n) & 1) * 0.5f;
            float q = (float)(o - n) / 2.0f;
            double r = Math.max((double)Mth.floor((double)(f - d)), j);
            double s = Math.min((double)Mth.ceil((double)(f + d)), k);
            float t = (float)(Mth.floor((double)r) & 1) * 0.5f;
    ...
    public void extract(WorldBorder worldBorder, float f, Vec3 vec3, double d, WorldBorderRenderState worldBorderRenderState) {
        ...
        worldBorderRenderState.alpha = Mth.clamp((double)worldBorderRenderState.alpha, (double)0.0, (double)1.0);
    ...
    public void render(WorldBorderRenderState worldBorderRenderState, Vec3 vec3, double d, double e) {
        ...
        double g = vec3.z;
        float h = (float)e;
        float i = (float)ARGB.red((int)worldBorderRenderState.tint) / 255.0f;
        float j = (float)ARGB.green((int)worldBorderRenderState.tint) / 255.0f;
        float k = (float)ARGB.blue((int)worldBorderRenderState.tint) / 255.0f;
        float l = (float)(Util.getMillis() % 3000L) / 3000.0f;
        float m = (float)(-Mth.frac((double)(vec3.y * 0.5)));
        float n = m + h;
