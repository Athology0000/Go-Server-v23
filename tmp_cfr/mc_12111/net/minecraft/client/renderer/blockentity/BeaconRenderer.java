...
implements BlockEntityRenderer<T, BeaconRenderState> {
    ...
    public static <T extends BlockEntity> void extract(T blockEntity, BeaconRenderState beaconRenderState, float f, Vec3 vec3) {
        beaconRenderState.animationTime = blockEntity.getLevel() != null ? (float)Math.floorMod(blockEntity.getLevel().getGameTime(), 40) + f : 0.0f;
        beaconRenderState.sections = ((BeaconBeamOwner)blockEntity).getBeamSections().stream().map(section -> new BeaconRenderState.Section(section.getColor(), section.getHeight())).toList();
    ...
    public static void submitBeaconBeam(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, Identifier identifier, float f, float g, int i, int j, int k, float h, float l) {
        ...
        float n = j < 0 ? g : -g;
        float o = Mth.frac((float)(n * 0.2f - (float)Mth.floor((float)(n * 0.1f))));
        poseStack.pushPose();
        ...
        z = -1.0f + o;
        aa = (float)j * f + z;
        submitNodeCollector.submitCustomGeometry(poseStack, RenderTypes.beaconBeam(identifier, true), (pose, vertexConsumer) -> BeaconRenderer.renderPart(pose, vertexConsumer, ARGB.color((int)32, (int)k), i, m, p, q, r, s, t, u, v, w, 0.0f, 1.0f, aa, z));
        poseStack.popPose();
    }
