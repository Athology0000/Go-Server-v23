...
extends AbstractEndPortalRenderer<TheEndGatewayBlockEntity, EndGatewayRenderState> {
    private static final Identifier BEAM_LOCATION = Identifier.withDefaultNamespace((String)"textures/entity/end_gateway_beam.png");
    ...
    public void extractRenderState(TheEndGatewayBlockEntity theEndGatewayBlockEntity, EndGatewayRenderState endGatewayRenderState, float f, Vec3 vec3,  @Nullable ModelFeatureRenderer.CrumblingOverlay crumblingOverlay) {
        ...
        if (theEndGatewayBlockEntity.isSpawning() || theEndGatewayBlockEntity.isCoolingDown() && level != null) {
            ...
            endGatewayRenderState.scale = Mth.sin((double)(endGatewayRenderState.scale * (float)Math.PI));
            endGatewayRenderState.height = Mth.floor((double)((double)endGatewayRenderState.scale * d));
            ...
            endGatewayRenderState.animationTime = theEndGatewayBlockEntity.getLevel() != null ? (float)Math.floorMod(theEndGatewayBlockEntity.getLevel().getGameTime(), 40) + f : 0.0f;
        } else {
