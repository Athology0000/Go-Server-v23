...
public final class TranslucencyPointOfView {
    ...
    public TranslucencyPointOfView set(Vec3 vec3, long l) {
        this.x = TranslucencyPointOfView.getCoordinate(vec3.x(), SectionPos.x((long)l));
        this.y = TranslucencyPointOfView.getCoordinate(vec3.y(), SectionPos.y((long)l));
        this.z = TranslucencyPointOfView.getCoordinate(vec3.z(), SectionPos.z((long)l));
        return this;
    }

    private static int getCoordinate(double d, int i) {
        int j = SectionPos.blockToSectionCoord((double)d) - i;
        return Mth.clamp((int)j, (int)-1, (int)1);
    }
