...
implements DebugRenderer.SimpleDebugRenderer {
    ...
    private void doRender(DebugValueAccess debugValueAccess) {
        ...
        debugValueAccess.forEachEntity(DebugSubscriptions.BEES, (entity, debugBeeInfo) -> {
            if (this.minecraft.player.closerThan((Entity)entity, 30.0)) {
                ...
                this.renderBeeInfo((Entity)entity, (DebugBeeInfo)debugBeeInfo, debugGoalInfo);
            }
        });
        ...
        debugValueAccess.forEachBlock(DebugSubscriptions.BEE_HIVES, (blockPos2, debugHiveInfo) -> {
            if (blockPos.closerThan((Vec3i)blockPos2, 30.0)) {
                ...
                Set<UUID> set = map.getOrDefault(blockPos2, Set.of());
                this.renderHiveInfo((BlockPos)blockPos2, (DebugHiveInfo)debugHiveInfo, (Collection<UUID>)set, debugValueAccess);
            }
        });
    ...
    private void renderFlowerInfos(DebugValueAccess debugValueAccess) {
        ...
        debugValueAccess.forEachEntity(DebugSubscriptions.BEES, (entity, debugBeeInfo) -> {
            if (debugBeeInfo.flowerPos().isPresent()) {
                map.computeIfAbsent((BlockPos)debugBeeInfo.flowerPos().get(), blockPos -> new HashSet()).add(entity.getUUID());
            }
        });
        map.forEach((blockPos, set) -> {
            Set set2 = set.stream().map(DebugEntityNameGenerator::getEntityName).collect(Collectors.toSet());
            int i = 1;
            Gizmos.billboardTextOverBlock((String)set2.toString(), (BlockPos)blockPos, (int)i++, (int)-256, (float)0.32f);
            Gizmos.billboardTextOverBlock((String)"Flower", (BlockPos)blockPos, (int)i++, (int)-1, (float)0.32f);
            Gizmos.cuboid((BlockPos)blockPos, (float)0.05f, (GizmoStyle)GizmoStyle.fill((int)ARGB.colorFromFloat((float)0.3f, (float)0.8f, (float)0.8f, (float)0.0f)));
        });
    }
    ...
    private void renderGhostHive(BlockPos blockPos, List<String> list) {
        ...
        Gizmos.billboardTextOverBlock((String)"Ghost Hive", (BlockPos)blockPos, (int)1, (int)-65536, (float)0.32f);
    }
    ...
    private void renderBeeInfo(Entity entity, DebugBeeInfo debugBeeInfo, @Nullable DebugGoalInfo debugGoalInfo) {
        ...
        int i = 0;
        Gizmos.billboardTextOverMob((Entity)entity, (int)i++, (String)debugBeeInfo.toString(), (int)-1, (float)0.48f);
        if (debugBeeInfo.hivePos().isEmpty()) {
            Gizmos.billboardTextOverMob((Entity)entity, (int)i++, (String)"No hive", (int)-98404, (float)0.32f);
        } else {
            Gizmos.billboardTextOverMob((Entity)entity, (int)i++, (String)("Hive: " + this.getPosDescription(entity, (BlockPos)debugBeeInfo.hivePos().get())), (int)-256, (float)0.32f);
        }
        if (debugBeeInfo.flowerPos().isEmpty()) {
            Gizmos.billboardTextOverMob((Entity)entity, (int)i++, (String)"No flower", (int)-98404, (float)0.32f);
        } else {
            Gizmos.billboardTextOverMob((Entity)entity, (int)i++, (String)("Flower: " + this.getPosDescription(entity, (BlockPos)debugBeeInfo.flowerPos().get())), (int)-256, (float)0.32f);
        }
        if (debugGoalInfo != null) {
            for (DebugGoalInfo.DebugGoal debugGoal : debugGoalInfo.goals()) {
                if (!debugGoal.isRunning()) continue;
                Gizmos.billboardTextOverMob((Entity)entity, (int)i++, (String)debugGoal.name(), (int)-16711936, (float)0.32f);
            }
        }
        if (debugBeeInfo.travelTicks() > 0) {
            int j = debugBeeInfo.travelTicks() < 2400 ? -3355444 : -23296;
            Gizmos.billboardTextOverMob((Entity)entity, (int)i++, (String)("Travelling: " + debugBeeInfo.travelTicks() + " ticks"), (int)j, (float)0.32f);
        }
    }

    private static void renderTextOverHive(String string, BlockPos blockPos, int i, int j) {
        Gizmos.billboardTextOverBlock((String)string, (BlockPos)blockPos, (int)i, (int)j, (float)0.32f);
    }
    ...
    private String getPosDescription(Entity entity, BlockPos blockPos) {
        double d = blockPos.distToCenterSqr((Position)entity.position());
    ...
    private Map<BlockPos, List<String>> getGhostHives(DebugValueAccess debugValueAccess) {
        ...
        debugValueAccess.forEachEntity(DebugSubscriptions.BEES, (entity, debugBeeInfo) -> {
            if (debugBeeInfo.hivePos().isPresent() && debugValueAccess.getBlockValue(DebugSubscriptions.BEE_HIVES, (BlockPos)debugBeeInfo.hivePos().get()) == null) {
                map.computeIfAbsent((BlockPos)debugBeeInfo.hivePos().get(), blockPos -> Lists.newArrayList()).add(DebugEntityNameGenerator.getEntityName((Entity)entity));
            }
        });
        return map;
    }
