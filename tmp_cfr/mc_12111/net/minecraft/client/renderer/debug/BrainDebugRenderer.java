...
implements DebugRenderer.SimpleDebugRenderer {
    ...
    private void doRender(DebugValueAccess debugValueAccess) {
        debugValueAccess.forEachEntity(DebugSubscriptions.BRAINS, (entity, debugBrainDump) -> {
            if (this.minecraft.player.closerThan((Entity)entity, 30.0)) {
                this.renderBrainInfo((Entity)entity, (DebugBrainDump)debugBrainDump);
            }
        });
    }
    ...
    private void renderBrainInfo(Entity entity, DebugBrainDump debugBrainDump) {
        ...
        int i = 0;
        Gizmos.billboardTextOverMob((Entity)entity, (int)i, (String)debugBrainDump.name(), (int)-1, (float)0.48f);
        ++i;
        if (bl) {
            Gizmos.billboardTextOverMob((Entity)entity, (int)i, (String)(debugBrainDump.profession() + " " + debugBrainDump.xp() + " xp"), (int)-1, (float)0.32f);
            ++i;
        }
        if (bl) {
            int j = debugBrainDump.health() < debugBrainDump.maxHealth() ? -23296 : -1;
            Gizmos.billboardTextOverMob((Entity)entity, (int)i, (String)("health: " + String.format(Locale.ROOT, "%.1f", Float.valueOf(debugBrainDump.health())) + " / " + String.format(Locale.ROOT, "%.1f", Float.valueOf(debugBrainDump.maxHealth()))), (int)j, (float)0.32f);
            ++i;
        }
        if (bl && !debugBrainDump.inventory().equals("")) {
            Gizmos.billboardTextOverMob((Entity)entity, (int)i, (String)debugBrainDump.inventory(), (int)-98404, (float)0.32f);
            ++i;
        }
        if (bl) {
            for (String string : debugBrainDump.behaviors()) {
                Gizmos.billboardTextOverMob((Entity)entity, (int)i, (String)string, (int)-16711681, (float)0.32f);
                ++i;
            }
        }
        if (bl) {
            for (String string : debugBrainDump.activities()) {
                Gizmos.billboardTextOverMob((Entity)entity, (int)i, (String)string, (int)-16711936, (float)0.32f);
                ++i;
            }
        }
        if (debugBrainDump.wantsGolem()) {
            Gizmos.billboardTextOverMob((Entity)entity, (int)i, (String)"Wants Golem", (int)-23296, (float)0.32f);
            ++i;
        }
        if (bl && debugBrainDump.angerLevel() != -1) {
            Gizmos.billboardTextOverMob((Entity)entity, (int)i, (String)("Anger Level: " + debugBrainDump.angerLevel()), (int)-98404, (float)0.32f);
            ++i;
        }
        if (bl) {
            for (String string : debugBrainDump.gossips()) {
                if (string.startsWith(debugBrainDump.name())) {
                    Gizmos.billboardTextOverMob((Entity)entity, (int)i, (String)string, (int)-1, (float)0.32f);
                } else {
                    Gizmos.billboardTextOverMob((Entity)entity, (int)i, (String)string, (int)-23296, (float)0.32f);
                }
                ++i;
            }
        }
        if (bl) {
            for (String string : Lists.reverse((List)debugBrainDump.memories())) {
                Gizmos.billboardTextOverMob((Entity)entity, (int)i, (String)string, (int)-3355444, (float)0.32f);
                ++i;
            }
        }
    }
    ...
    public Map<BlockPos, List<String>> getGhostPois(DebugValueAccess debugValueAccess) {
        ...
        debugValueAccess.forEachEntity(DebugSubscriptions.BRAINS, (entity, debugBrainDump) -> {
            for (BlockPos blockPos2 : Iterables.concat((Iterable)debugBrainDump.pois(), (Iterable)debugBrainDump.potentialPois())) {
