...
implements DebugRenderer.SimpleDebugRenderer {
    ...
    private static void renderFace(Direction direction, int i, int j, int k, int l) {
        Vec3 vec3 = new Vec3((double)SectionPos.sectionToBlockCoord((int)i), (double)SectionPos.sectionToBlockCoord((int)j), (double)SectionPos.sectionToBlockCoord((int)k));
        Vec3 vec32 = vec3.add(16.0, 16.0, 16.0);
        Gizmos.rect((Vec3)vec3, (Vec3)vec32, (Direction)direction, (GizmoStyle)GizmoStyle.fill((int)l));
    }

    private static void renderEdge(int i, int j, int k, int l, int m, int n, int o) {
        double d = SectionPos.sectionToBlockCoord((int)i);
        double e = SectionPos.sectionToBlockCoord((int)j);
        double f = SectionPos.sectionToBlockCoord((int)k);
        double g = SectionPos.sectionToBlockCoord((int)l);
        double h = SectionPos.sectionToBlockCoord((int)m);
        double p = SectionPos.sectionToBlockCoord((int)n);
        int q = ARGB.opaque((int)o);
        Gizmos.line((Vec3)new Vec3(d, e, f), (Vec3)new Vec3(g, h, p), (int)q);
    }
    ...
    static final class SectionData {
        ...
        SectionData(LevelLightEngine levelLightEngine, SectionPos sectionPos, int i, LightLayer lightLayer) {
            ...
            for (int k = 0; k < j; ++k) {
                for (int l = 0; l < j; ++l) {
                    for (int m = 0; m < j; ++m) {
                        SectionPos sectionPos2 = SectionPos.of((int)(sectionPos.x() + m - i), (int)(sectionPos.y() + l - i), (int)(sectionPos.z() + k - i));
            ...
            this.minPos = SectionPos.of((int)(sectionPos.x() - i), (int)(sectionPos.y() - i), (int)(sectionPos.z() - i));
        }
    }
}
