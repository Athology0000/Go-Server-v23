...
implements DebugRenderer.SimpleDebugRenderer {
    ...
    public void emitGizmos(double d, double e, double f, DebugValueAccess debugValueAccess, Frustum frustum, float g) {
        ...
        if (h - this.lastUpdateTime > 1.0E8) {
            ...
            this.surroundEntities = ImmutableList.copyOf((Collection)entity.level().getEntities(entity, entity.getBoundingBox().inflate(16.0)));
        }
    ...
    private void drawHighlights(Entity entity, DoubleSupplier doubleSupplier, int i) {
        entity.mainSupportingBlockPos.ifPresent(blockPos -> {
            ...
            if (!blockPos3.equals((Object)blockPos2)) {
    ...
    private void highlightPosition(BlockPos blockPos, double d, int i) {
        ...
        Gizmos.cuboid((AABB)new AABB(e, f, g, h, j, k), (GizmoStyle)GizmoStyle.stroke((int)ARGB.color((float)0.4f, (int)i)));
        VoxelShape voxelShape = this.minecraft.level.getBlockState(blockPos).getCollisionShape((BlockGetter)this.minecraft.level, blockPos, CollisionContext.empty()).move((Vec3i)blockPos);
        GizmoStyle gizmoStyle = GizmoStyle.stroke((int)i);
