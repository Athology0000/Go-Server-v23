...
extends LivingEntityRenderer<ArmorStand, ArmorStandRenderState, ArmorStandArmorModel> {
    public static final Identifier DEFAULT_SKIN_LOCATION = Identifier.withDefaultNamespace((String)"textures/entity/armorstand/wood.png");
    ...
    public void extractRenderState(ArmorStand armorStand, ArmorStandRenderState armorStandRenderState, float f) {
        ...
        HumanoidMobRenderer.extractHumanoidRenderState((LivingEntity)armorStand, armorStandRenderState, f, this.itemModelResolver);
        armorStandRenderState.yRot = Mth.rotLerp((float)f, (float)armorStand.yRotO, (float)armorStand.getYRot());
    ...
    protected void setupRotations(ArmorStandRenderState armorStandRenderState, PoseStack poseStack, float f, float g) {
        ...
        if (armorStandRenderState.wiggle < 5.0f) {
            poseStack.mulPose((Quaternionfc)Axis.YP.rotationDegrees(Mth.sin((double)(armorStandRenderState.wiggle / 1.5f * (float)Math.PI)) * 3.0f));
        }
    }
