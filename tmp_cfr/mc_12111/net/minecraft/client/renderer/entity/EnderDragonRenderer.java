...
extends EntityRenderer<EnderDragon, EnderDragonRenderState> {
    ...
    private static final Identifier DRAGON_LOCATION = Identifier.withDefaultNamespace((String)"textures/entity/enderdragon/dragon.png");
    ...
    public void submit(EnderDragonRenderState enderDragonRenderState, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, CameraRenderState cameraRenderState) {
        ...
        if (enderDragonRenderState.deathTime > 0.0f) {
            int j = ARGB.white((float)(enderDragonRenderState.deathTime / 200.0f));
    ...
    private static void submitRays(PoseStack poseStack, float f, SubmitNodeCollector submitNodeCollector, RenderType renderType) {
        submitNodeCollector.submitCustomGeometry(poseStack, renderType, (pose, vertexConsumer) -> {
            ...
            int i = ARGB.colorFromFloat((float)(1.0f - g), (float)1.0f, (float)1.0f, (float)1.0f);
            int j = 0xFF00FF;
    ...
    public static void submitCrystalBeams(float f, float g, float h, float i, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int j) {
        float k = Mth.sqrt((float)(f * f + h * h));
        float l = Mth.sqrt((float)(f * f + g * g + h * h));
        poseStack.pushPose();
        ...
        submitNodeCollector.submitCustomGeometry(poseStack, BEAM, (pose, vertexConsumer) -> {
            ...
            float m = 0.0f;
            for (int n = 1; n <= 8; ++n) {
                float o = Mth.sin((double)((float)n * ((float)Math.PI * 2) / 8.0f)) * 0.75f;
                float p = Mth.cos((double)((float)n * ((float)Math.PI * 2) / 8.0f)) * 0.75f;
    ...
    public void extractRenderState(EnderDragon enderDragon, EnderDragonRenderState enderDragonRenderState, float f) {
        ...
        enderDragonRenderState.flapTime = Mth.lerp((float)f, (float)enderDragon.oFlapTime, (float)enderDragon.flapTime);
        ...
        BlockPos blockPos = enderDragon.level().getHeightmapPos(Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, EndPodiumFeature.getLocation((BlockPos)enderDragon.getFightOrigin()));
        enderDragonRenderState.distanceToEgg = blockPos.distToCenterSqr((Position)enderDragon.position());
