    ...
    public void submit(IllusionerRenderState illusionerRenderState, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, CameraRenderState cameraRenderState) {
        if (illusionerRenderState.isInvisible) {
            ...
            for (int i = 0; i < vec3s.length; ++i) {
                poseStack.pushPose();
                poseStack.translate(vec3s[i].x + (double)Mth.cos((double)((float)i + illusionerRenderState.ageInTicks * 0.5f)) * 0.025, vec3s[i].y + (double)Mth.cos((double)((float)i + illusionerRenderState.ageInTicks * 0.75f)) * 0.0125, vec3s[i].z + (double)Mth.cos((double)((float)i + illusionerRenderState.ageInTicks * 0.7f)) * 0.025);
