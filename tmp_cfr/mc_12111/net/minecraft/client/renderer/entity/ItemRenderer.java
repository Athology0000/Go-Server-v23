...
public class ItemRenderer {
    ...
    public static List<RenderType> getFoilRenderTypes(RenderType renderType, boolean bl, boolean bl2) {
        if (bl2) {
            if (ItemRenderer.useTransparentGlint(renderType)) {
                return List.of(renderType, RenderTypes.glintTranslucent());
            }
            return List.of(renderType, bl ? RenderTypes.glint() : RenderTypes.entityGlint());
        }
    ...
    private static void renderQuadList(PoseStack poseStack, VertexConsumer vertexConsumer, List<BakedQuad> list, int[] is, int i, int j) {
        ...
        for (BakedQuad bakedQuad : list) {
            ...
            if (bakedQuad.isTinted()) {
                ...
                f = (float)ARGB.alpha((int)k) / 255.0f;
                g = (float)ARGB.red((int)k) / 255.0f;
                h = (float)ARGB.green((int)k) / 255.0f;
                l = (float)ARGB.blue((int)k) / 255.0f;
            } else {
                f = 1.0f;
                g = 1.0f;
                h = 1.0f;
