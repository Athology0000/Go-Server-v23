...
extends AgeableMobRenderer<Panda, PandaRenderState, PandaModel> {
    private static final Map<Panda.Gene, Identifier> TEXTURES = Maps.newEnumMap(Map.of(Panda.Gene.NORMAL, Identifier.withDefaultNamespace((String)"textures/entity/panda/panda.png"), Panda.Gene.LAZY, Identifier.withDefaultNamespace((String)"textures/entity/panda/lazy_panda.png"), Panda.Gene.WORRIED, Identifier.withDefaultNamespace((String)"textures/entity/panda/worried_panda.png"), Panda.Gene.PLAYFUL, Identifier.withDefaultNamespace((String)"textures/entity/panda/playful_panda.png"), Panda.Gene.BROWN, Identifier.withDefaultNamespace((String)"textures/entity/panda/brown_panda.png"), Panda.Gene.WEAK, Identifier.withDefaultNamespace((String)"textures/entity/panda/weak_panda.png"), Panda.Gene.AGGRESSIVE, Identifier.withDefaultNamespace((String)"textures/entity/panda/aggressive_panda.png")));
    ...
    public void extractRenderState(Panda panda, PandaRenderState pandaRenderState, float f) {
        ...
        HoldingEntityRenderState.extractHoldingEntityRenderState((LivingEntity)panda, pandaRenderState, this.itemModelResolver);
    ...
    protected void setupRotations(PandaRenderState pandaRenderState, PoseStack poseStack, float f, float g) {
        ...
        if (pandaRenderState.rollTime > 0.0f) {
            float l;
            h = Mth.frac((float)pandaRenderState.rollTime);
            int i = Mth.floor((float)pandaRenderState.rollTime);
            int j = i + 1;
            float k = 7.0f;
        ...
        if ((h = pandaRenderState.sitAmount) > 0.0f) {
            ...
            poseStack.mulPose((Quaternionfc)Axis.XP.rotationDegrees(Mth.lerp((float)h, (float)pandaRenderState.xRot, (float)(pandaRenderState.xRot + 90.0f))));
        ...
        if ((q = pandaRenderState.lieOnBackAmount) > 0.0f) {
            ...
            poseStack.mulPose((Quaternionfc)Axis.XP.rotationDegrees(Mth.lerp((float)q, (float)pandaRenderState.xRot, (float)(pandaRenderState.xRot + 180.0f))));
        }
    }

    private float getAngle(float f, float g, int i, float h, float j) {
        if ((float)i < j) {
            return Mth.lerp((float)h, (float)f, (float)g);
        }
        return f;
    }
