...
extends RenderLayer<AvatarRenderState, M> {
    ...
    public void submit(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int i, AvatarRenderState avatarRenderState, float f, float g) {
        ...
        RandomSource randomSource = RandomSource.create((long)avatarRenderState.id);
        for (int k = 0; k < j; ++k) {
            poseStack.pushPose();
            ...
            poseStack.translate(Mth.lerp((float)h, (float)cube.minX, (float)cube.maxX) / 16.0f, Mth.lerp((float)l, (float)cube.minY, (float)cube.maxY) / 16.0f, Mth.lerp((float)m, (float)cube.minZ, (float)cube.maxZ) / 16.0f);
