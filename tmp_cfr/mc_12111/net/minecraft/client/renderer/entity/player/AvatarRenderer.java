...
extends LivingEntityRenderer<AvatarlikeEntity, AvatarRenderState, PlayerModel> {
    ...
    private static HumanoidModel.ArmPose getArmPose(Avatar avatar, ItemStack itemStack, InteractionHand interactionHand) {
        ...
        if (!avatar.swinging && itemStack.is(Items.CROSSBOW) && CrossbowItem.isCharged((ItemStack)itemStack)) {
        ...
        if ((swingAnimation = (SwingAnimation)itemStack.get(DataComponents.SWING_ANIMATION)) != null && swingAnimation.type() == SwingAnimationType.STAB && avatar.swinging) {
            return HumanoidModel.ArmPose.SPEAR;
        }
    ...
    public void extractRenderState(AvatarlikeEntity avatar, AvatarRenderState avatarRenderState, float f) {
        ...
        if (avatarRenderState.isUsingItem && (itemStack = avatar.getItemInHand(avatarRenderState.useItemHand)).is(Items.SPYGLASS)) {
    ...
    private void extractFlightData(AvatarlikeEntity avatar, AvatarRenderState avatarRenderState, float f) {
        avatarRenderState.fallFlyingTimeInTicks = (float)avatar.getFallFlyingTicks() + f;
        Vec3 vec3 = avatar.getViewVector(f);
        Vec3 vec32 = ((ClientAvatarEntity)avatar).avatarState().deltaMovementOnPreviousTick().lerp(avatar.getDeltaMovement(), (double)f);
    ...
    private void extractCapeState(AvatarlikeEntity avatar, AvatarRenderState avatarRenderState, float f) {
        ...
        double d = clientAvatarState.getInterpolatedCloakX(f) - Mth.lerp((double)f, (double)((Avatar)avatar).xo, (double)avatar.getX());
        double e = clientAvatarState.getInterpolatedCloakY(f) - Mth.lerp((double)f, (double)((Avatar)avatar).yo, (double)avatar.getY());
        double g = clientAvatarState.getInterpolatedCloakZ(f) - Mth.lerp((double)f, (double)((Avatar)avatar).zo, (double)avatar.getZ());
        float h = Mth.rotLerp((float)f, (float)((Avatar)avatar).yBodyRotO, (float)((Avatar)avatar).yBodyRot);
        double i = Mth.sin((double)(h * ((float)Math.PI / 180)));
        double j = -Mth.cos((double)(h * ((float)Math.PI / 180)));
        avatarRenderState.capeFlap = (float)e * 10.0f;
        avatarRenderState.capeFlap = Mth.clamp((float)avatarRenderState.capeFlap, (float)-6.0f, (float)32.0f);
        avatarRenderState.capeLean = (float)(d * i + g * j) * 100.0f;
        avatarRenderState.capeLean *= 1.0f - avatarRenderState.fallFlyingScale();
        avatarRenderState.capeLean = Mth.clamp((float)avatarRenderState.capeLean, (float)0.0f, (float)150.0f);
        avatarRenderState.capeLean2 = (float)(d * j - g * i) * 100.0f;
        avatarRenderState.capeLean2 = Mth.clamp((float)avatarRenderState.capeLean2, (float)-20.0f, (float)20.0f);
        float k = clientAvatarState.getInterpolatedBob(f);
        float l = clientAvatarState.getInterpolatedWalkDistance(f);
        avatarRenderState.capeFlap += Mth.sin((double)(l * 6.0f)) * 32.0f * k;
    }
    ...
    protected void setupRotations(AvatarRenderState avatarRenderState, PoseStack poseStack, float f, float g) {
        ...
        if (avatarRenderState.isFallFlying) {
            ...
            float k = Mth.lerp((float)h, (float)0.0f, (float)j);
