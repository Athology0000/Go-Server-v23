...
extends FogEnvironment {
    ...
    public int getBaseColor(ClientLevel clientLevel, Camera camera, int i, float f) {
        ...
        if (i >= 4) {
            int l;
            float m;
            float g = ((Float)camera.attributeProbe().getValue(EnvironmentAttributes.SUN_ANGLE, f)).floatValue() * ((float)Math.PI / 180);
            h = Mth.sin((double)g) > 0.0f ? -1.0f : 1.0f;
            ...
            if (k > 0.0f && (m = ARGB.alphaFloat((int)(l = ((Integer)camera.attributeProbe().getValue(EnvironmentAttributes.SUNRISE_SUNSET_COLOR, f)).intValue()))) > 0.0f) {
                j = ARGB.srgbLerp((float)(k * m), (int)j, (int)ARGB.opaque((int)l));
            }
        }
        ...
        h = Math.min(((Float)camera.attributeProbe().getValue(EnvironmentAttributes.SKY_FOG_END_DISTANCE, f)).floatValue() / 16.0f, (float)i);
        float o = Mth.clampedLerp((float)(h / 32.0f), (float)0.25f, (float)1.0f);
        o = 1.0f - (float)Math.pow(o, 0.25);
        j = ARGB.srgbLerp((float)o, (int)j, (int)n);
        return j;
    }
    ...
    private static int applyWeatherDarken(int i, float f, float g) {
        if (f > 0.0f) {
            float h = 1.0f - f * 0.5f;
            float j = 1.0f - f * 0.4f;
            i = ARGB.scaleRGB((int)i, (float)h, (float)h, (float)j);
        }
        if (g > 0.0f) {
            i = ARGB.scaleRGB((int)i, (float)(1.0f - g * 0.5f));
        }
        return i;
    }
    ...
    public void setupFog(FogData fogData, Camera camera, ClientLevel clientLevel, float f, DeltaTracker deltaTracker) {
        ...
        fogData.environmentalStart = ((Float)camera.attributeProbe().getValue(EnvironmentAttributes.FOG_START_DISTANCE, g)).floatValue();
        fogData.environmentalEnd = ((Float)camera.attributeProbe().getValue(EnvironmentAttributes.FOG_END_DISTANCE, g)).floatValue();
        ...
        fogData.skyEnd = Math.min(f, ((Float)camera.attributeProbe().getValue(EnvironmentAttributes.SKY_FOG_END_DISTANCE, g)).floatValue());
        fogData.cloudEnd = Math.min((float)(Minecraft.getInstance().options.cloudRange().get() * 16), ((Float)camera.attributeProbe().getValue(EnvironmentAttributes.CLOUD_FOG_END_DISTANCE, g)).floatValue());
    ...
    private void updateRainFogState(Camera camera, ClientLevel clientLevel, DeltaTracker deltaTracker) {
        ...
        float h = Mth.clamp((float)(((float)clientLevel.getLightEngine().getLayerListener(LightLayer.SKY).getLightValue(blockPos) - 8.0f) / 7.0f), (float)0.0f, (float)1.0f);
