...
extends NeedleDirectionHelper {
    ...
    private float getRandomlySpinningRotation(int i, long l) {
        ...
        float f = this.noTargetWobbler.rotation() + (float)CompassAngleState.hash(i) / 2.1474836E9f;
    ...
    private static boolean isValidCompassTargetPos(ItemOwner itemOwner, @Nullable GlobalPos globalPos) {
        return globalPos != null && globalPos.dimension() == itemOwner.level().dimension() && !(globalPos.pos().distToCenterSqr((Position)itemOwner.position()) < (double)1.0E-5f);
    }
