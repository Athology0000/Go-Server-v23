...
implements RangeSelectItemModelProperty {
    ...
    public static enum TimeSource implements StringRepresentable
    {
        ...
        DAYTIME("daytime"){
            ...
            @Override
            public float get(ClientLevel clientLevel, ItemStack itemStack, ItemOwner itemOwner, RandomSource randomSource) {
                return ((Float)clientLevel.environmentAttributes().getValue(EnvironmentAttributes.SUN_ANGLE, itemOwner.position())).floatValue() / 360.0f;
            }
        }
        ,
        MOON_PHASE("moon_phase"){
            ...
            @Override
            public float get(ClientLevel clientLevel, ItemStack itemStack, ItemOwner itemOwner, RandomSource randomSource) {
                return (float)((MoonPhase)clientLevel.environmentAttributes().getValue(EnvironmentAttributes.MOON_PHASE, itemOwner.position())).index() / (float)MoonPhase.COUNT;
            }
        };
