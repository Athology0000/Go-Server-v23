...
AutoCloseable {
    ...
    public SpriteContents(Identifier identifier, FrameSize frameSize, NativeImage nativeImage, Optional<AnimationMetadataSection> optional, List<MetadataSectionType.WithValue<?>> list, Optional<TextureMetadataSection> optional2) {
        ...
        this.animatedTexture = optional.map(animationMetadataSection -> this.createAnimatedTexture(frameSize, nativeImage.getWidth(), nativeImage.getHeight(), (AnimationMetadataSection)animationMetadataSection)).orElse(null);
    ...
    public void increaseMipLevel(int i) {
        try {
            ...
            CrashReport crashReport = CrashReport.forThrowable((Throwable)throwable, (String)"Generating mipmaps for frame");
            CrashReportCategory crashReportCategory = crashReport.addCategory("Frame being iterated");
            crashReportCategory.setDetail("Sprite name", (Object)this.name);
            ...
            crashReportCategory.setDetail("Mipmap levels", (Object)i);
    ...
    public <T> Optional<T> getAdditionalMetadata(MetadataSectionType<T> metadataSectionType) {
        for (MetadataSectionType.WithValue<?> withValue : this.additionalMetadata) {
            Optional optional = withValue.unwrapToType(metadataSectionType);
    ...
    public boolean isTransparent(int i, int j, int k) {
        ...
        return ARGB.alpha((int)this.originalImage.getPixel(l, m)) == 0;
    }
