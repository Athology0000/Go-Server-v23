...
public class SpriteLoader {
    ...
    private Preparations stitch(List<SpriteContents> list, int i, Executor executor) {
        try (Zone zone = Profiler.get().zone(() -> "stitch " + String.valueOf(this.location));){
            ...
            for (SpriteContents spriteContents : list) {
                ...
                if (m >= l) continue;
                LOGGER.warn("Texture {} with size {}x{} limits mip level from {} to {}", new Object[]{spriteContents.name(), spriteContents.width(), spriteContents.height(), Mth.log2((int)l), Mth.log2((int)m)});
                l = m;
            }
            int n = Math.min(k, l);
            int o = Mth.log2((int)n);
            if (o < i) {
            ...
            try {
                ...
                CrashReport crashReport = CrashReport.forThrowable((Throwable)stitcherException, (String)"Stitching");
                CrashReportCategory crashReportCategory = crashReport.addCategory("Stitcher");
                crashReportCategory.setDetail("Sprites", (Object)stitcherException.getAllSprites().stream().map(entry -> String.format(Locale.ROOT, "%s[%dx%d]", entry.name(), entry.width(), entry.height())).collect(Collectors.joining(",")));
                crashReportCategory.setDetail("Max Texture Size", (Object)j);
    ...
    private static CompletableFuture<List<SpriteContents>> runSpriteSuppliers(SpriteResourceLoader spriteResourceLoader, List<SpriteSource.Loader> list2, Executor executor) {
        List<@Nullable CompletableFuture> list22 = list2.stream().map(loader -> CompletableFuture.supplyAsync(() -> loader.get(spriteResourceLoader), executor)).toList();
