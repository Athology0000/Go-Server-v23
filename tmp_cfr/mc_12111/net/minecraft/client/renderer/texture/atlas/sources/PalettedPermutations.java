...
public record PalettedPermutations(List<Identifier> textures, Identifier paletteKey, Map<String, Identifier> permutations, String separator) implements SpriteSource
{
    ...
    public static final MapCodec<PalettedPermutations> MAP_CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)Codec.list((Codec)Identifier.CODEC).fieldOf("textures").forGetter(PalettedPermutations::textures), (App)Identifier.CODEC.fieldOf("palette_key").forGetter(PalettedPermutations::paletteKey), (App)Codec.unboundedMap((Codec)Codec.STRING, (Codec)Identifier.CODEC).fieldOf("permutations").forGetter(PalettedPermutations::permutations), (App)Codec.STRING.optionalFieldOf("separator", (Object)DEFAULT_SEPARATOR).forGetter(PalettedPermutations::separator)).apply((Applicative)instance, PalettedPermutations::new));
    ...
    private static IntUnaryOperator createPaletteMapping(int[] is, int[] js) {
        ...
        for (int i = 0; i < is.length; ++i) {
            int j = is[i];
            if (ARGB.alpha((int)j) == 0) continue;
            int2IntMap.put(ARGB.transparent((int)j), js[i]);
        }
    ...
    private static int[] loadPaletteEntryFromImage(ResourceManager resourceManager, Identifier identifier) {
        Optional optional = resourceManager.getResource(TEXTURE_ID_CONVERTER.idToFile(identifier));
        if (optional.isEmpty()) {
    ...
    private static /* synthetic */ int method_48489(Int2IntMap int2IntMap, int i) {
        int j = ARGB.alpha((int)i);
        if (j == 0) {
            return i;
        }
        int k = ARGB.transparent((int)i);
        int l = int2IntMap.getOrDefault(k, ARGB.opaque((int)k));
        int m = ARGB.alpha((int)l);
        return ARGB.color((int)(j * m / 255), (int)l);
    }
